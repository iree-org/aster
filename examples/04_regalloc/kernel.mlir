...
amdgcn.module @regalloc target = <gfx942> {
  amdgcn.kernel @kernel arguments <[
    ...
    %data, %tok_ld = amdgcn.global_load_dword outs(%data_reg) ins(%src, offset = %off_reg) args(%c0)
        : outs(!amdgcn.vgpr) ins(!amdgcn.sgpr<[? : ? + 2]>, offset = !amdgcn.vgpr<11>) args(i32) -> !amdgcn.read_token<flat>
    ...
    // Store result
    %tok_st = amdgcn.global_store_dword ins(%result, %dst, offset = %off_reg) args(%c0)
        : ins(!amdgcn.vgpr, !amdgcn.sgpr<[? : ? + 2]>, offset = !amdgcn.vgpr<11>) args(i32) -> !amdgcn.write_token<flat>
