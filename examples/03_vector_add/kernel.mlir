...
module {
  amdgcn.module @vadd target = <gfx942> {
    ...
    func.func private @load_3_ptrs(%kernarg: !amdgcn.sgpr<[0 : 2]>)
      ...
      amdgcn.s_load_dwordx2 outs(%a) ins(%kernarg) args(%c0)
          : outs(!amdgcn.sgpr<[2 : 4]>) ins(!amdgcn.sgpr<[0 : 2]>) args(i32) -> !amdgcn.read_token<constant>
      amdgcn.s_load_dwordx2 outs(%b) ins(%kernarg) args(%c8)
          : outs(!amdgcn.sgpr<[4 : 6]>) ins(!amdgcn.sgpr<[0 : 2]>) args(i32) -> !amdgcn.read_token<constant>
      amdgcn.s_load_dwordx2 outs(%c) ins(%kernarg) args(%c16)
          : outs(!amdgcn.sgpr<[6 : 8]>) ins(!amdgcn.sgpr<[0 : 2]>) args(i32) -> !amdgcn.read_token<constant>
    ...
    kernel @kernel arguments <[
      ...
      %c0 = arith.constant 0 : i32
      amdgcn.global_load_dword outs(%a_data) ins(%a_ptr, offset = %v1) args(%c0)
          : outs(!amdgcn.vgpr<[2 : 3]>) ins(!amdgcn.sgpr<[2 : 4]>, offset = !amdgcn.vgpr<1>) args(i32) -> !amdgcn.read_token<flat>

      %b_data = amdgcn.make_register_range %v3 : !amdgcn.vgpr<3>
      amdgcn.global_load_dword outs(%b_data) ins(%b_ptr, offset = %v1) args(%c0)
          : outs(!amdgcn.vgpr<[3 : 4]>) ins(!amdgcn.sgpr<[4 : 6]>, offset = !amdgcn.vgpr<1>) args(i32) -> !amdgcn.read_token<flat>
      ...
      amdgcn.global_store_dword ins(%c_data, %c_ptr, offset = %v1) args(%c0)
          : ins(!amdgcn.vgpr<[2 : 3]>, !amdgcn.sgpr<[6 : 8]>, offset = !amdgcn.vgpr<1>) args(i32) -> !amdgcn.write_token<flat>
