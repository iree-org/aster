...
user feedback while collaborating in the open. For the next few weeks, expect a
lot of churn and evolution, before we can stabilize and start accepting PRs.
...
#### Executing pytests
```
(cd build && ninja install) && pytest ./examples/ ./integration_test/
```
...
- **Not multi-vendor portable**: ASTER is specifically designed for AMD GPUs
and their ISA. While architectural principles are applicable elsewhere,
cross-vendor portability is an explicit non-goal. This focus enables deeper
specialization and better exploitation of AMD-specific features.

- **Not a high-level programming language**: ASTER operates at the assembly/IR
level. While Python APIs improve ergonomics, the tool assumes users want low-level
control. Higher-level abstractions should be built on top of ASTER, not within it.
...
Seb Vince's feedback was instrumental in performance debugging of the early work.
