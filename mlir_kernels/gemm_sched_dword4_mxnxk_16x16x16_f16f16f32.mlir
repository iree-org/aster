...
amdgcn.module @kernel_module target = #amdgcn.target<gfx942> isa = #amdgcn.isa<cdna3> {
  ...
  // Perform MFMA: load fragments, compute, store result
  ...
  func.func private @matmul_loop(
    ...
    // Loop over remaining 4-D tile **distributed** tile index (K, d_MMNNKK):
    %ub = affine.apply affine_map<()[K, d_MMNNKK] -> (K * d_MMNNKK)> ()[%K, %d_MMNNKK]
    scf.for %idx = %c0 to %ub step %c1 {
      // Decompose linear index into 3D index
      %k, %d_mmnnkk = affine.delinearize_index %idx into (%K, %d_MMNNKK) : index, index, index
      ...
      func.call @maybe_store_c_fragment(
        %d_mmnnkk, %d_MMNN, %KK, %w, %W, %MM, %NN, %k, %K,
