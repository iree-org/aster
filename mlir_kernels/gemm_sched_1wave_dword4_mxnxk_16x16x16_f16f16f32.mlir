...
//===----------------------------------------------------------------------===//
// Type Aliases - from descriptors.mlir
//===----------------------------------------------------------------------===//
...
// Index pair for grid partitioning
!index_pair = !aster_utils.struct<i: index, j: index>

// Tensor position descriptors (global memory)
!tensor_position_descriptor_2d = !aster_utils.struct<
  ptr: !sx2, m_pos: index, n_pos: index,
  global_stride_in_bytes: index, elt_size: index>

!tensor_position_descriptor_2level_2d = !aster_utils.struct<
  ptr: !sx2, m_pos: index, n_pos: index, global_stride_in_bytes: index,
  mm_pos: index, nn_pos: index, elt_size: index>

// LDS position descriptors
!lds_position_descriptor_2d = !aster_utils.struct<
  lds_base: index, m_pos: index, n_pos: index,
  lds_stride_in_bytes: index, elt_size: index>

!lds_position_descriptor_2level_2d = !aster_utils.struct<
  lds_base: index, mm_pos: index, nn_pos: index,
  lds_stride_in_bytes: index, elt_size: index>

// Transfer and conditional execution descriptors
!transfer_descriptor_2d = !aster_utils.struct<
...
!future_global_read_any = !aster_utils.struct<
  value: !aster_utils.any, token: !amdgcn.read_token<flat>>
!future_lds_write = !amdgcn.write_token<shared>
!future_lds_read_any = !aster_utils.struct<
  value: !aster_utils.any, token: !amdgcn.read_token<shared>>

// 4D tile index descriptor: k, mm, nn, kk
!tile_index_descriptor_4d = !aster_utils.struct<
  k: index, mm: index, nn: index, kk: index>

// 4D tile dimensions descriptor: K, MM, NN, KK
!tile_dims_descriptor_4d = !aster_utils.struct<
  K: index, MM: index, NN: index, KK: index>

// C fragment conditional execution descriptor
!store_conditional_execution_descriptor_2d = !aster_utils.struct<
  k: index, kk: index, K: index, KK: index>

// C fragment position descriptor
!c_fragment_position_descriptor_2d = !aster_utils.struct<
  mm: index, nn: index>
...
amdgcn.module @kernel_module target = #amdgcn.target<gfx942> isa = #amdgcn.isa<cdna3> {
  ...
  // From copies.mlir - basic operations
  ...
  func.func private @maybe_init_wave_16x16xf32_C_fragment(
    !store_conditional_execution_descriptor_2d,
    !c_fragment_position_descriptor_2d,
    memref<?x?x!vx4>)
  ...
  func.func private @maybe_get_global_load_values_vx2(
    index, index, memref<?x?x!vx2>, memref<?x!future_global_read_any>, i1)

  func.func private @maybe_get_lds_read_value_vx2(
    index, index, index, memref<?x?x?x!vx2>,
    memref<?x?x?x!future_lds_read_any>, i1)
  ...
  //===--------------------------------------------------------------------===//
  // Main Matmul Loop Function
  //===--------------------------------------------------------------------===//

  func.func private @matmul_loop(
    %SIZE_M: index, %SIZE_N: index, %SIZE_K: index,
    %TILE_SIZE_M: index, %TILE_SIZE_N: index, %TILE_SIZE_K: index,
    %a_global: !sx2, %b_global: !sx2, %c_global: !sx2
  ) {
    //------------------------------------------------------------------
    // GPU variables and constants
    //------------------------------------------------------------------
    ...
    //------------------------------------------------------------------
    // Block-level tiling: partition grid
    //------------------------------------------------------------------
    ...
    %result = func.call @tiled_grid_partition_2d(%sizes, %tile_sizes)
      : (!index_pair, !index_pair) -> !index_pair
    ...
    //------------------------------------------------------------------
    // Warp-level tile sizes (16x16 MFMA tiles)
    //------------------------------------------------------------------
    ...
    // LDS read fragment buffers
    %a_frag_memref = memref.alloca(%K, %MM, %KK) : memref<?x?x?x!vx2>
    %b_frag_memref = memref.alloca(%K, %NN, %KK) : memref<?x?x?x!vx2>

    // Future memrefs for async execution
    %a_future_memref = memref.alloca(%NT_MK) : memref<?x!future_global_read_any>
    %b_future_memref = memref.alloca(%NT_NK) : memref<?x!future_global_read_any>

    // LDS write token memrefs
    %a_lds_token_memref = memref.alloca(%NT_MK) : memref<?x!future_lds_write>
    %b_lds_token_memref = memref.alloca(%NT_NK) : memref<?x!future_lds_write>

    // LDS read future memrefs
    %a_lds_read_future_memref = memref.alloca(%K, %MM, %KK) : memref<?x?x?x!future_lds_read_any>
    %b_lds_read_future_memref = memref.alloca(%K, %NN, %KK) : memref<?x?x?x!future_lds_read_any>

    //------------------------------------------------------------------
    // Main computation loop: iterate over K, MM, NN, KK
    //------------------------------------------------------------------
    scf.for %k = %c0 to %K step %c1 {
      %ub = affine.apply affine_map<()[MM, NN, KK] -> (MM * NN * KK)>()[%MM, %NN, %KK]
      ...
      scf.for %idx = %c0 to %ub step %c1 {
        ...
        %tensor_desc_a = aster_utils.struct_create(
        ...
        %tensor_desc_b = aster_utils.struct_create(
        ...
        func.call @maybe_get_global_load_values_vx2(
          %k, %NT_MK, %a_load_memref, %a_future_memref, %a_valid)
          : (index, index, memref<?x?x!vx2>, memref<?x!future_global_read_any>, i1) -> ()
        func.call @maybe_get_global_load_values_vx2(
          %k, %NT_NK, %b_load_memref, %b_future_memref, %b_valid)
          : (index, index, memref<?x?x!vx2>, memref<?x!future_global_read_any>, i1) -> ()

        // LDS stride = TILE_SIZE_K * sizeof(f16)
        %elt_size_lds = arith.constant 2 : index
        %lds_stride_bytes = affine.apply
        ...
        %cond_desc_init_c = aster_utils.struct_create(%k, %kk, %K, %KK)
          : (index, index, index, index) -> !store_conditional_execution_descriptor_2d
        %pos_desc_init_c = aster_utils.struct_create(%mm, %nn)
          : (index, index) -> !c_fragment_position_descriptor_2d
        func.call @maybe_init_wave_16x16xf32_C_fragment(
          %cond_desc_init_c, %pos_desc_init_c, %c_fragments)
          {sched.delay = 0 : i64, sched.rate = 1 : i64}
          : (!store_conditional_execution_descriptor_2d,
             !c_fragment_position_descriptor_2d, memref<?x?x!vx4>) -> ()

        //==============================================================
        // Stage 4: LDS reads (wait for DS writes)
        //==============================================================

        // Wait for A DS write, then read A
        func.call @maybe_wait_all_lds_writes(%a_lds_valid, %a_lds_token_memref)
        ...
        func.call @maybe_get_lds_read_value_vx2(
          %k, %mm, %kk, %a_frag_memref, %a_lds_read_future_memref, %a_lds_read_valid)
          {sched.delay = 10 : i64, sched.rate = 1 : i64}
          : (index, index, index, memref<?x?x?x!vx2>,
             memref<?x?x?x!future_lds_read_any>, i1) -> ()
        func.call @maybe_get_lds_read_value_vx2(
          %k, %nn, %kk, %b_frag_memref, %b_lds_read_future_memref, %b_lds_read_valid)
          {sched.delay = 10 : i64, sched.rate = 1 : i64}
          : (index, index, index, memref<?x?x?x!vx2>,
             memref<?x?x?x!future_lds_read_any>, i1) -> ()

        // Execute MFMA
        %idx_desc = aster_utils.struct_create(%k, %mm, %nn, %kk)
          : (index, index, index, index) -> !tile_index_descriptor_4d
        func.call @maybe_mfma(%idx_desc, %a_frag_memref, %b_frag_memref, %c_fragments)
          {sched.delay = 10 : i64, sched.rate = 1 : i64}
          : (!tile_index_descriptor_4d, memref<?x?x?x!vx2>,
             memref<?x?x?x!vx2>, memref<?x?x!vx4>) -> ()

        //==============================================================
        // Stage 6: Store C fragment (last K iteration only)
        //==============================================================

        %elt_size_c = arith.constant 4 : index  // f32 size in bytes
        %GLOBAL_C_STRIDE_IN_BYTES = affine.apply
          affine_map<()[SIZE_N, elt_sz] -> (SIZE_N * elt_sz)>()[%SIZE_N, %elt_size_c]

        %cond_desc_store_c = aster_utils.struct_create(%k, %kk, %K, %KK)
          : (index, index, index, index) -> !store_conditional_execution_descriptor_2d
        %tensor_desc_c = aster_utils.struct_create(
          %c_global, %m_pos, %n_pos, %GLOBAL_C_STRIDE_IN_BYTES, %mm, %nn, %elt_size_c)
          : (!sx2, index, index, index, index, index, index)
          -> !tensor_position_descriptor_2level_2d
        func.call @maybe_global_store_wave_16x16xf32_C_fragment(
          %cond_desc_store_c, %tensor_desc_c, %c_fragments)
          {sched.delay = 12 : i64, sched.rate = 1 : i64}
          : (!store_conditional_execution_descriptor_2d,
             !tensor_position_descriptor_2level_2d, memref<?x?x!vx4>) -> ()
  ...
  amdgcn.kernel @test_matmul_kernel arguments <[
    ...
    shared_memory_size = {{LDS_SIZE}} : i32,
