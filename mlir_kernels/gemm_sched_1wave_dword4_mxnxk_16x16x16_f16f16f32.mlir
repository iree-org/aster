...
amdgcn.module @kernel_module target = #amdgcn.target<gfx942> isa = #amdgcn.isa<cdna3> {
  ...
  func.func private @maybe_global_load_A(
    ...
    %i_pos: index, %j_pos: index, %k_pos: index, %SIZE_K: index, // global positions
    %a_load_memref: memref<?x?x!vx2>,                             // memref for decoupled global load
    %a_frag_memref: memref<?x?x!vx2>                             // tmp memref for avoiding LDS
  ) {
    ...
    scf.if %is_nn_zero {
      ...
      %loaded = func.call @global_load_dwordx2_wait(%a_global, %i_pos, %k_pos, %SIZE_K, %mm_pos, %c0, %KK)
        : (!sx2, index, index, index, index, index, index) -> (!vx2)
      memref.store %loaded, %a_load_memref[%k, %mmkk] : memref<?x?x!vx2>

      // Store to tmp memref for avoiding LDS for now.
      %mmnnkk = affine.linearize_index [%mm, %nn, %kk] by (%MM, %NN, %KK) : index
      memref.store %loaded, %a_frag_memref[%k, %mmnnkk] : memref<?x?x!vx2>
    }
    return
  }
  ...
  func.func private @maybe_global_load_B(
    ...
    %i_pos: index, %j_pos: index, %k_pos: index, %SIZE_K: index, // global positions
    %b_load_memref: memref<?x?x!vx2>,                             // memref for decoupled global load
    %b_frag_memref: memref<?x?x!vx2>                             // tmp memref for avoiding LDS
  ) {
    %c0 = arith.constant 0 : index
    ...
    scf.if %is_mm_zero {
      ...
      %loaded = func.call @global_load_dwordx2_wait(%b_global, %j_pos, %k_pos, %SIZE_K, %nn_pos, %c0, %KK)
        : (!sx2, index, index, index, index, index, index) -> (!vx2)
      memref.store %loaded, %b_load_memref[%k, %nnkk] : memref<?x?x!vx2>

      // Store to tmp memref for avoiding LDS for now.
      %mmnnkk = affine.linearize_index [%mm, %nn, %kk] by (%MM, %NN, %KK) : index
      memref.store %loaded, %b_frag_memref[%k, %mmnnkk] : memref<?x?x!vx2>
    }
    return
  }
  ...
  //   %k: index, %d_mmnnkk: index, %d_MMNN: index, %KK: index,
  ...
  func.func private @maybe_mfma(
    %k: index, %mm: index, %nn: index, %kk: index,  // indices
    %K: index, %MM: index, %NN: index,  %KK: index, // sizes
    %a_frag_memref: memref<?x?x!vx2>,    // register fragments for A
    %b_frag_memref: memref<?x?x!vx2>,    // register fragments for B
    %c_fragments: memref<?x!vx4>         // register fragments for C
  ) {
    // Load fragments from memrefs
    %mmnnkk = affine.linearize_index [%mm, %nn, %kk] by (%MM, %NN, %KK) : index
    %a_frag = memref.load %a_frag_memref[%k, %mmnnkk] : memref<?x?x!vx2>
    %b_frag = memref.load %b_frag_memref[%k, %mmnnkk] : memref<?x?x!vx2>

    // Perform MFMA operation: C = A * B + C
    %mmnn = affine.linearize_index [%mm, %nn] by (%MM, %NN) : index
    %acc = memref.load %c_fragments[%mmnn] : memref<?x!vx4>
    %updated_acc = amdgcn.vop3p.vop3p_mai <v_mfma_f32_16x16x16_f16>
      %acc, %a_frag, %b_frag, %acc : !vx2, !vx2, !vx4 -> !vx4
    memref.store %updated_acc, %c_fragments[%mmnn] : memref<?x!vx4>
    return
  }
  ...
  func.func private @maybe_store_c_fragment(
    ...
    %i_pos: index, %j_pos: index, %SIZE_N: index    // global positions
  ) {
    %c1 = arith.constant 1 : index
    ...
    scf.if %is_last_k_and_kk {
      ...
      func.call @store_global_16x16xf32_C_fragment_wait(%fragment, %c_global, %i_pos, %j_pos, %SIZE_N, %mm_pos, %nn_pos)
        : (!vx4, !sx2, index, index, index, index, index) -> ()
    }
    return
  }
  ...
  func.func private @matmul_loop(
    ...
    %MMNNKK = affine.apply affine_map<()[sz0, sz1, sz2] -> (sz0 * sz1 * sz2)>()[%MM, %NN, %KK]
    %MMKK = affine.apply affine_map<()[sz0, sz1] -> (sz0 * sz1)>()[%MM, %KK]
    %NNKK = affine.apply affine_map<()[sz0, sz1] -> (sz0 * sz1)>()[%NN, %KK]
    %MMNN = affine.apply affine_map<()[sz0, sz1] -> (sz0 * sz1)>()[%MM, %NN]

    // Allocate registers for the C fragment
    %c_fragments = memref.alloca(%MMNN) : memref<?x!vx4>

    // Allocate memrefs for decoupled global loads -> DS writes
    %a_load_memref = memref.alloca(%K, %MMKK) : memref<?x?x!vx2>
    %b_load_memref = memref.alloca(%K, %NNKK) : memref<?x?x!vx2>

    // Allocate memrefs for decoupled LDS reads -> mfma
    %a_frag_memref = memref.alloca(%K, %MMNNKK) : memref<?x?x!vx2>
    %b_frag_memref = memref.alloca(%K, %MMNNKK) : memref<?x?x!vx2>

    // Initialize C fragments
    %c0_i32 = arith.constant 0 : i32
    scf.for %c = %c0 to %MMNN step %c1 {
    ...
    // Loop over remaining 4-D tile tile index (K, MMNNKK):
    %ub = affine.apply affine_map<()[K, MMNNKK] -> (K * MMNNKK)> ()[%K, %MMNNKK]
    scf.for %idx = %c0 to %ub step %c1 {
      // Decompose linear index into 4-D index
      ...
      func.call @maybe_global_load_A(
        ...
        %K, %MM, %NN, %KK,
        %a_global,
        %i_pos, %j_pos, %k_pos, %SIZE_K,
        %a_load_memref, %a_frag_memref)
      ...
      func.call @maybe_global_load_B(
        ...
        %b_global,
        %i_pos, %j_pos, %k_pos, %SIZE_K,
        %b_load_memref, %b_frag_memref)
        {sched.delay = 0 : i64, sched.rate = 1 : i64}
        : (index, index, index, index, index, index, index, index,
           !sx2, index, index, index, index,
           memref<?x?x!vx2>, memref<?x?x!vx2>) -> ()

      // // DS writes (decoupled from global loads via memrefs)
      // func.call @maybe_lds_write(
      //   %k, %d_mmnnkk, %NN, %MM, %d_MMKK, %d_NNKK, %w, %W, %KK,
      ...
      // // LDS reads (decoupled from mfma via memrefs)
      // func.call @maybe_lds_read(
      //   %k, %d_mmnnkk, %d_MMNN, %KK, %w, %W, %MM, %NN,
      //   %lds_a_base_off, %lds_b_base_off, %TILE_SIZE_K,
      ...
      func.call @maybe_store_c_fragment(
        %k, %mm, %nn, %kk,
        %K, %MM, %NN, %KK,
        %c_fragments, %c_global,
        %i_pos, %j_pos, %SIZE_N)
          {sched.delay = 8 : i64, sched.rate = 1 : i64}
        : (index, index, index, index, index, index, index, index,
           memref<?x!vx4>, !sx2, index, index, index) -> ()
