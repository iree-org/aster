...
amdgcn.module @test_uniform_loop target = <gfx942> isa = <cdna3> {
  kernel @test_uniform_loop arguments <[#amdgcn.buffer_arg<address_space = generic, access = read_only>, #amdgcn.buffer_arg<address_space = generic>]> {
    ...
    %result, %token = load s_load_dword dest %2 addr %0 : dps(!amdgcn.sgpr) ins(!amdgcn.sgpr_range<[? + 2]>) -> !amdgcn.read_token<constant>
    ...
    scf.for %arg0 = %c0_i32 to %3 step %c1_i32 : i32 {
      ...
      %8 = amdgcn.store global_store_dword data %7 addr %1 offset d(%7) : ins(!amdgcn.vgpr, !amdgcn.sgpr_range<[? + 2]>, !amdgcn.vgpr) -> !amdgcn.write_token<flat>
    }
    end_kernel
  }
}
