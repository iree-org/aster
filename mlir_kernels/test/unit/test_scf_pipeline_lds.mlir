...
amdgcn.module @test_lds_passthrough target = <gfx942> {
  kernel @test_lds_passthrough arguments <[
    ...
    scf.for %i = %c0 to %c4 step %c1 {
      ...
      %wtok = amdgcn.ds_write_b32 ins(%lds_addr, %val) args(%c0_i32) {sched.stage = 0 : i32}
          : ins(!v, !v) args(i32) -> !amdgcn.write_token<shared>
      ...
      %from_lds, %rtok = amdgcn.ds_read_b32 outs(%r_dest) ins(%lds_addr) args(%c0_i32_mig1) {sched.stage = 1 : i32}
          : outs(!v) ins(!v) args(i32) -> !amdgcn.read_token<shared>
      ...
      %stok = amdgcn.global_store_dword ins(%data, %out_ptr, offset = %off_reg) args(%c0_i32_mig1) {sched.stage = 1 : i32}
...
amdgcn.module @test_lds_iv_dep target = <gfx942> {
  kernel @test_lds_iv_dep arguments <[
    ...
    scf.for %i = %c0 to %c8 step %c1 {
      ...
      %lds_off = amdgcn.get_lds_offset %lds {sched.stage = 0 : i32} : i32
      %lds_addr = lsir.to_reg %lds_off {sched.stage = 0 : i32} : i32 -> !v
      %wtok = amdgcn.ds_write_b32 ins(%lds_addr, %val_reg) args(%c0_i32) {sched.stage = 0 : i32}
          : ins(!v, !v) args(i32) -> !amdgcn.write_token<shared>
      ...
      %from_lds, %rtok = amdgcn.ds_read_b32 outs(%r_dest) ins(%lds_addr) args(%c0_i32_mig2) {sched.stage = 1 : i32}
          : outs(!v) ins(!v) args(i32) -> !amdgcn.read_token<shared>
      amdgcn.wait deps %rtok {sched.stage = 1 : i32} : !amdgcn.read_token<shared>
      ...
      %data = amdgcn.v_mov_b32 outs(%d_store) ins(%from_lds) {sched.stage = 1 : i32} : outs(!v) ins(!v)
      %stok = amdgcn.global_store_dword ins(%data, %out_ptr, offset = %off_reg) args(%c0_i32_mig2) {sched.stage = 1 : i32}
          : ins(!v, !sx2, offset = !v) args(i32) -> !amdgcn.write_token<flat>
...
amdgcn.module @test_lds_six_stage target = <gfx942> {
  kernel @test_lds_six_stage arguments <[
    ...
    scf.for %i = %c0 to %c10 step %c1 {
      ...
      %c0_i32_mig1 = arith.constant 0 : i32
      %from_global, %gltok = amdgcn.global_load_dword outs(%d_load) ins(%in_ptr, offset = %off_reg) args(%c0_i32_mig1) {sched.stage = 0 : i32}
          : outs(!v) ins(!sx2, offset = !v) args(i32) -> !amdgcn.read_token<flat>
      ...
      %wtok_a = amdgcn.ds_write_b32 ins(%lds_a_addr, %from_global) args(%c0_i32) {sched.stage = 1 : i32}
          : ins(!v, !v) args(i32) -> !amdgcn.write_token<shared>
      ...
      %from_a, %rtok_a = amdgcn.ds_read_b32 outs(%d_ra) ins(%lds_a_addr) args(%c0_i32_mig3) {sched.stage = 2 : i32}
          : outs(!v) ins(!v) args(i32) -> !amdgcn.read_token<shared>

      // Stage 3: ds_write to LDS_B
      amdgcn.wait deps %rtok_a {sched.stage = 3 : i32} : !amdgcn.read_token<shared>
      %wtok_b = amdgcn.ds_write_b32 ins(%lds_b_addr, %from_a) args(%c0_i32) {sched.stage = 3 : i32}
          : ins(!v, !v) args(i32) -> !amdgcn.write_token<shared>
      ...
      %from_b, %rtok_b = amdgcn.ds_read_b32 outs(%d_rb) ins(%lds_b_addr) args(%c0_i32_mig4) {sched.stage = 4 : i32}
          : outs(!v) ins(!v) args(i32) -> !amdgcn.read_token<shared>
      ...
      %stok = amdgcn.global_store_dword ins(%data, %out_ptr, offset = %off_reg_s5) args(%c0_i32_mig3) {sched.stage = 5 : i32}
          : ins(!v, !sx2, offset = !v) args(i32) -> !amdgcn.write_token<flat>
...
amdgcn.module @test_lds_accum target = <gfx942> {
  kernel @test_lds_accum arguments <[
    ...
    %final_acc = scf.for %i = %c0 to %c6 step %c1
      ...
      %from_lds, %rtok = amdgcn.ds_read_b32 outs(%r_dest) ins(%lds_addr) args(%c0_i32_mig5) {sched.stage = 1 : i32}
          : outs(!v) ins(!v) args(i32) -> !amdgcn.read_token<shared>
      amdgcn.wait deps %rtok {sched.stage = 1 : i32} : !amdgcn.read_token<shared>
    ...
    %c0_i32_mig4 = arith.constant 0 : i32
    %stok = amdgcn.global_store_dword ins(%final_acc, %out_ptr, offset = %off_reg) args(%c0_i32_mig4)
        : ins(!v, !sx2, offset = !v) args(i32) -> !amdgcn.write_token<flat>
    amdgcn.s_waitcnt vmcnt = 0
    end_kernel
  }
