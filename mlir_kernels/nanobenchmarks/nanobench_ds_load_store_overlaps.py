...
KERNEL_NAME = "nanobench_ds_load_store_overlaps"
...
def main():
    ...
    mlir_file = os.path.join(script_dir, "nanobench_ds_load_store_overlaps.mlir")
    ...
    print(f"\n{'='*60}")
    print(f"Multi-tile DS load/store overlaps nanobenchmark")
    print(f"{'='*60}")
    print(f"\nConfiguration:")
    print(f"  Tiles: {args.num_tiles_i}x{args.num_tiles_j} = {num_tiles} tiles")
    print(f"  LDS size: {lds_size} bytes")
    print(f"  Input buffer: {buffer_size} bytes ({num_elements} f16 elements)")
    ...
    print(f"\nExpected pattern for {num_tiles} tiles:")
    print(f"  - {num_tiles} vmcnt waits (one per global_load -> ds_write)")
    print(f"  - {num_tiles} lgkmcnt waits (one per ds_write -> ds_read)")
    print(f"  - {num_tiles} lgkmcnt waits (one per ds_read -> use)")
    print(f"  - Optimal: vmcnt={num_tiles-1}, {num_tiles-2}, ..., 0")
