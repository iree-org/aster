...
amdgcn.module @kernel_module target = #amdgcn.target<gfx942> isa = #amdgcn.isa<cdna3> {
  ...
  func.func private @global_load_body(
    ...
    // Workgroup level:
    //   read from [bidx] with stride [ NUM_WAVES * szI * szJ * 16 * 16 * 2] bytes.
    // Wave level:
    ...
    %offset = affine.apply affine_map<
      (bidx, widx, i, j, lidx)[num_waves, szI, szJ]
        -> (bidx * num_waves * szI * szJ * 16 * 16 * 2 + 
                        widx * szI * szJ * 16 * 16 * 2 + 
                                 i * szJ * 16 * 16 * 2 +
                                       j * 16 * 16 * 2 +
                                       lidx * 8)>
      (%bidx, %widx, %i, %j, %lidx)[%num_waves, %szI, %szJ]
    %offset_vgpr = pir.to_reg %offset : index -> !amdgcn.vgpr
  ...
  func.func private @global_store_body(
    ...
    // Workgroup level:
    //   write to [bidx] with stride [ NUM_WAVES * szI * szJ * 16 * 16 * 4] bytes.
    // Wave level:
    //   write to [waveidx_x] with stride [ szI * szJ * 16 * 16 * 4] bytes.
    // Thread level:
    //   write to [i, j] with stride [ szJ * 16 * 16 * 4, 16 * 16 * 4] bytes.
    //   write to [lidx] with stride [ (16 * 16 * 4) / 64 = 16] bytes.
    %num_waves = affine.apply affine_map<()[bdimx] -> (bdimx floordiv 64)>()[%bdimx]
    %widx = affine.apply affine_map<()[tidx] -> (tidx floordiv 64)>()[%tidx]
    %lidx = affine.apply affine_map<()[tidx] -> (tidx mod 64)>()[%tidx]
    %offset = affine.apply affine_map<
      (bidx, widx, i, j, lidx)[num_waves, szI, szJ]
        -> (bidx * num_waves * szI * szJ * 16 * 16 * 4 + 
                        widx * szI * szJ * 16 * 16 * 4 + 
                                 i * szJ * 16 * 16 * 4 +
                                       j * 16 * 16 * 4 +
                                       lidx * 16)>
      (%bidx, %widx, %i, %j, %lidx)[%num_waves, %szI, %szJ]
  ...
  func.func private @matmul_loop(%M: index, %N: index, %K: index,
    ...
    scf.for %i = %c0 to %MNK step %c1 {
      ...
      scf.if %is_first_n {
        func.call @global_load_body(%tidx, %bidx, %bdimx, %gdimx, %m, %k, %M, %K, %a_global, %a_memref)
      ...
      scf.if %is_first_m {
        func.call @global_load_body(%tidx, %bidx, %bdimx, %gdimx, %k, %n, %K, %N, %b_global, %b_memref)
          : (index, index, index, index, index, index, index, index, !amdgcn.sgpr_range<[? + 2]>, memref<?x?x!amdgcn.vgpr_range<[? + 2]>>) -> ()
      }
      ...
      // scf.if %is_first_n {
      //   func.call @ds_write_body(%tidx, %bidx, %bdimx, %gdimx, %m, %k, %M, %K, %a_memref, %c0_i32)
      ...
      // scf.if %is_first_m {
      //   func.call @ds_write_body(%tidx, %bidx, %bdimx, %gdimx, %k, %n, %K, %N, %b_memref, %clds_b_shift_i32)
      //     : (index, index, index, index, index, index, index, index, memref<?x?x!amdgcn.vgpr_range<[? + 2]>>, i32) -> ()
      // }

      // // Part 1c: DS read for A
      // scf.if %is_first_n {
      //   func.call @ds_read_body(%tidx, %bidx, %bdimx, %gdimx, %m, %k, %M, %K, %a_memref, %c0_i32)
      //     : (index, index, index, index, index, index, index, index, memref<?x?x!amdgcn.vgpr_range<[? + 2]>>, i32) -> ()
      // }
      // // Part 2c: DS read for B
      // scf.if %is_first_m {
      //   func.call @ds_read_body(%tidx, %bidx, %bdimx, %gdimx, %k, %n, %K, %N, %b_memref, %clds_b_shift_i32)
      //     : (index, index, index, index, index, index, index, index, memref<?x?x!amdgcn.vgpr_range<[? + 2]>>, i32) -> ()
      // }
      ...
      scf.if %is_last_k {
        func.call @global_store_body(%tidx, %bidx, %bdimx, %gdimx, %m, %n, %M, %N, %c_memref, %c_global)
  ...
  amdgcn.kernel @test_matmul_kernel arguments <[
    ...
    %a_ptr, %b_ptr, %c_ptr = pir.assume_noalias %a_ptr_s, %b_ptr_s, %c_ptr_s
