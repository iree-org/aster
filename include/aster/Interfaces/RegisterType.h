...
public:
  Register() = default;
  explicit Register(int16_t number) : regNumber(number) {}

  /// Check if the register is valid.
  bool isValid() const { return isRelocatable() || getRegister() >= 0; }

  /// Check if the register is relocatable.
  bool isRelocatable() const { return !regNumber.has_value(); }

  /// Get the register number.
  int16_t getRegister() const {
    assert(regNumber.has_value() && "register is not set");
    return *regNumber;
  }

  /// Get a relocatable register.
  static Register getAsRelocatable() { return Register(); }
  ...
  friend llvm::hash_code hash_value(const Register &reg) {
    return llvm::hash_value(reg.regNumber);
  }

  /// Compare registers by number.
  bool operator<(const Register &other) const {
    if (isRelocatable() && other.isRelocatable())
      return false;
    if (!isRelocatable() && !other.isRelocatable())
      return *regNumber < *other.regNumber;
  ...
  Register getWithOffset(int16_t offset) const {
    return isRelocatable() ? Register() : Register(*regNumber + offset);
  }

private:
  std::optional<int16_t> regNumber;
};
...
public:
  ...
  /// Get the register range as a relocatable range.
  RegisterRange getAsRelocatableRange() const {
    return RegisterRange(begin().getAsRelocatable(), size(), indexAlignment);
  }
