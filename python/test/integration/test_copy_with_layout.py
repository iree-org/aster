...
def _build_copy_kernel(name, layout, swizzle=None, target=MCPU, isa="cdna3"):
    """Build copy kernel: load dwordx4 at layout(+swizzle) offset, store linear."""
    ...
    src_off = b.layout_byte_offset(tid, layout)
    if swizzle is not None:
        src_off = layout_d.swizzle(
            src_off,
            bits=swizzle.bits,
            base=swizzle.base,
            shift=swizzle.shift,
...
def _run_copy_test(name, layout, swizzle=None):
    """Load from layout(+swizzle)-ordered src, store to linear dst, verify."""
    ...
    with ctx:
        module = _build_copy_kernel(name, layout, swizzle)
...
def test_copy_linear():
    ...
    _run_copy_test("copy_lin", LINEAR)


def test_copy_tiled():
    """Tiled layout: 4 tiles of 16 threads, first-mode-slowest."""
    tiled = Layout(sizes=(4, 16), strides=(16 * ELEM_BYTES, ELEM_BYTES))
    _run_copy_test("copy_tile", tiled)


def test_copy_swizzled():
    """Linear + swizzle: layout.linearize + layout.swizzle on source."""
    # Note: internally ELEM_BYTES=16 and we need a Swizzle base >= log2(ELEM_BYTES)
    swizzled = Swizzle(bits=2, base=4, shift=6)
    _run_copy_test("copy_swz", LINEAR, swizzled)
