...
static bool cmpRanges(const Range &lhs, const Range &rhs) {
  auto getTuple = [](const Range &r) {
    RegisterTypeInterface ty = r.getRegisterType();
    return std::make_tuple(ty.getRegisterKindAsInt(),
                           ty.getRegisterKind().getAsOpaquePointer(),
                           r.getVariableIds().size(), r.getVariableIds(),
                           ty.getAsOpaquePointer(), r.getOp().getOperation());
  };
...
void RangeAnalysisImpl::createGraph() {
  for (size_t i = 0; i < ranges.size(); ++i) {
    for (size_t j = i + 1; j < ranges.size(); ++j) {
      if (ranges[i].getRegisterType().getRegisterKind() !=
          ranges[j].getRegisterType().getRegisterKind())
        break;
