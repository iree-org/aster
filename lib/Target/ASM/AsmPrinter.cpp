...
static void printRegister(llvm::raw_ostream &os, RegisterTypeInterface type) {
  RegisterRange range = type.getAsRange();
  StringRef prefix;
  switch (cast<RegisterKindAttr>(type.getRegisterKind()).getValue()) {
...
void aster::amdgcn::AsmPrinter::printOperand(Value operand) {
  ...
  printRegister(os, cast<RegisterTypeInterface>(operand.getType()));
}
