//===- DecomposeOps.cpp - Decompose addi/muli ops -------------------------===//
//
...
#define DEBUG_TYPE "decompose-ops"

namespace mlir::aster {
namespace aster_utils {
#define GEN_PASS_DEF_DECOMPOSEOPS
...
struct DecomposeOps : public aster_utils::impl::DecomposeOpsBase<DecomposeOps> {
  using Base::Base;
...
void DecomposeOps::runOnOperation() {
  IRRewriter rewriter(&getContext());

  // Step 1: Split LICM operands into separate inner ops.
  ...
  // Step 2: Hoist the loop-invariant inner ops and their passthrough barriers.
  getOperation()->walk([&](LoopLikeOpInterface loopLike) {
    moveLoopInvariantCode(
        ...
        [&](Operation *op, Region *) {
          return isa<AddiOp, MuliOp, aster_utils::PassthroughOp>(op) ||
                 op->hasTrait<OpTrait::ConstantLike>();
        },
        [&](Operation *op, Region *) { loopLike.moveOutOfLoop(op); });
  });

  // Step 3: Block-level common sub-expression decomposition.
