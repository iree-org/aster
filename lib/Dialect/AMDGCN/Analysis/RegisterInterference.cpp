//===- RegisterInterference.cpp - Register interference graph analysis ---===//
//
...
RegisterInterference::NodeID
RegisterInterference::getOrCreateNodeId(Value value) {
...
RegisterInterference::NodeID
RegisterInterference::getNodeId(Value value) const {
  return valueToNodeId.lookup_or(value, -1);
}

Value RegisterInterference::getValue(NodeID nodeId) const {
  if (nodeId < 0 || static_cast<size_t>(nodeId) >= values.size())
    return nullptr;
  return values[nodeId];
}

void RegisterInterference::addEdges(Value lhs, Value rhs) {
  if (lhs == rhs)
    return;
...
LogicalResult RegisterInterference::handleOp(Operation *op) {
...
LogicalResult RegisterInterference::run(Operation *op) {
...
FailureOr<RegisterInterference>
RegisterInterference::create(Operation *op, DataFlowSolver &solver,
  ...
  // Build the graph.
  RegisterInterference graph(solver);
...
void RegisterInterference::print(raw_ostream &os) const {
