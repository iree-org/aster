...
#include "aster/Dialect/AMDGCN/Analysis/RegisterInterference.h"
#include "aster/Dialect/AMDGCN/Analysis/RegisterRangeAnalysis.h"
...
struct RegisterAllocator {
  using NodeId = RegisterInterference::NodeID;
  RegisterAllocator(RegisterInterference &graph,
                    const RangeConstraintAnalysis &analysis, MLIRContext *ctx)
      : graph(graph), analysis(analysis), rewriter(ctx) {}
...
private:
  ...
  RegisterInterference &graph;
  const RangeConstraintAnalysis &analysis;
...
LogicalResult RegisterAllocator::alloc(Value alloca) {
  ...
  // register.
  if (const RangeConstraint *constraint = analysis.lookup(alloca)) {
  ...
  for (auto [i, alloca] : llvm::enumerate(allocas)) {
    ...
    graph.getValues()[graph.getNodeId(alloca)] = newAlloca.getResult();
  }
  return success();
}
...
LogicalResult RegisterAllocator::run(Operation *op) {
  ...
  for (auto [i, node] : llvm::enumerate(nodes)) {
    ...
    if (visited.insert(i).second == false || !needsAllocation(node))
      continue;
...
LogicalResult RegisterColoring::run(FunctionOpInterface funcOp) {
  ...
  if (failed(solver.initializeAndRun(funcOp))) {
    return funcOp.emitError() << "Failed to run liveness analysis";
  }
  ...
  FailureOr<RegisterInterference> graph =
      RegisterInterference::create(funcOp, solver, symbolTable);
  if (failed(graph))
    return failure();

  // Create and run the register allocator.
  RegisterAllocator allocator(*graph, *analysis, funcOp->getContext());
  ...
  if (failed(applyPatternsGreedily(funcOp, frozenPatterns,
                                   GreedyRewriteConfig()))) {
