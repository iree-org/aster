...
static std::optional<RegisterRange> getVGPRRange(Value value) {
  ...
  if (regType.getRegisterKindAsInt() !=
      static_cast<int64_t>(RegisterKind::VGPR))
    return std::nullopt;
...
static std::optional<RegisterRange> getSGPRRange(Value value) {
  ...
  if (regType.getRegisterKindAsInt() !=
      static_cast<int64_t>(RegisterKind::SGPR))
    return std::nullopt;
...
struct NopInsertionCaseDef {
  ...
  NopInsertionCaseDef(std::function<bool(Operation *)> s1,
                      ...
                      int maxLookaheadCount, int caseNum, NopType nop)
...
/// Case 106: "XDL Write VGPR or V_SMFMA* Write VGPR","1) VM, L/GDS, FLAT, Export Read VGPR overlapped with 1st vDst 2) VALU read/write VGPR (RAW + WAW)","5 if 1st V_MFMA is 2 passes
static NopInsertionCaseDef getCase106Definition() {
  ...
  auto selectInst2 = [](Operation *op) -> bool {
    return isa<inst::GlobalStoreOp, inst::DSWriteOp, inst::VOP2Op, inst::VOP1Op>(op);
  };
...
void AMDGCNNopInsertion::runOnOperation() {
  ...
  for (const NopInsertionCase &nopCase : allCases) {
    ...
    if (nopCase.nopType == NopType::SNOP) {
