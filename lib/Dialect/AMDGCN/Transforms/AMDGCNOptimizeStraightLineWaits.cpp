...
void AMDGCNOptimizeStraightLineWaits::runOnOperation() {
  ...
  block->walk([&](Operation *op) {
    ...
    for (const auto &memOp : beforeState->getPendingAfterOp()) {
      if (memOp.resourceType == GlobalMemoryResource::get(ctx))
        pendingFlushVmcnt++;
      if (memOp.resourceType == LDSMemoryResource::get(ctx))
        pendingFlushLgkmcnt++;
    }
    ...
    Operation *sWaitcntOp =
        S_WAITCNT::create(b, op->getLoc(), vmcnt, expcnt, lgkmcnt);
    LDBG() << "Inserted waitcnt: " << *sWaitcntOp;
  });

  // At the end of the pass, replace pir.assume_noalias results with its
  // operands: just forward the operands This serves as a poor man's
  // "late lowering" of pir.assume_noalias ops.
  SmallVector<Operation *> assumeNoaliasOpsToErase;
  block->walk([&](Operation *op) {
    if (isa<pir::AssumeNoaliasOp>(op)) {
