...
RegisterKind mlir::aster::amdgcn::getRegisterKind(RegisterTypeInterface type) {
  return cast<RegisterKindAttr>(type.getRegisterKind()).getValue();
}
...
struct MaxRegisterCount;

template <>
struct MaxRegisterCount<SGPRType> {
  static int get(AMDGCNDialect *dialect) { return dialect->maxSGPRRegisters; }
};

template <>
struct MaxRegisterCount<VGPRType> {
  static int get(AMDGCNDialect *dialect) { return dialect->maxVGPRRegisters; }
};

template <>
struct MaxRegisterCount<AGPRType> {
  static int get(AMDGCNDialect *dialect) { return dialect->maxAGPRRegisters; }
};

template <typename RegisterType, typename ResourceType>
class RegisterResourceManager {
public:
  static RegisterResourceManager &getInstance(MLIRContext *ctx) {
    // Use function-local static with initialization - thread-safe in C++11
    static RegisterResourceManager instance(ctx);
    return instance;
  }

  ResourceType *getResource(RegisterType type) {
    auto regType = cast<RegisterTypeInterface>(type);
    if (regType.isRelocatable())
      llvm::report_fatal_error("Cannot get resource for relocatable register");
    RegisterRange range = regType.getAsRange();
    int regNum = range.begin().getRegister();
    auto it = resources.find(regNum);
    assert(it != resources.end() && "Resource not found");
    return it->second.get();
  }

private:
  RegisterResourceManager(MLIRContext *ctx) {
    auto *dialect = ctx->getLoadedDialect<AMDGCNDialect>();
    assert(dialect && "AMDGCN dialect not loaded");

    int maxRegisters = MaxRegisterCount<RegisterType>::get(dialect);
    for (int i = 0; i < maxRegisters; ++i) {
      RegisterType type = RegisterType::get(ctx, Register(i));
      resources[i] = std::make_unique<ResourceType>(type);
    }
  }

public:
  ~RegisterResourceManager() = default;

private:
  RegisterResourceManager(const RegisterResourceManager &) = delete;
  RegisterResourceManager &operator=(const RegisterResourceManager &) = delete;

  llvm::DenseMap<int, std::unique_ptr<ResourceType>> resources;
};

using SGPRResourceManager = RegisterResourceManager<SGPRType, SGPRResource>;
using VGPRResourceManager = RegisterResourceManager<VGPRType, VGPRResource>;
using AGPRResourceManager = RegisterResourceManager<AGPRType, AGPRResource>;
} // namespace

SGPRResource *SGPRResource::get(SGPRType type) {
  return SGPRResourceManager::getInstance(type.getContext()).getResource(type);
}

VGPRResource *VGPRResource::get(VGPRType type) {
  return VGPRResourceManager::getInstance(type.getContext()).getResource(type);
}

AGPRResource *AGPRResource::get(AGPRType type) {
  return AGPRResourceManager::getInstance(type.getContext()).getResource(type);
}
...
GlobalMemoryResource *GlobalMemoryResource::get(MLIRContext *ctx) {
  // Use function-local static with initialization - thread-safe in C++11
  static GlobalMemoryResource instance;
  return &instance;
}

LDSMemoryResource *LDSMemoryResource::get(MLIRContext *ctx) {
  // Use function-local static with initialization - thread-safe in C++11
  static LDSMemoryResource instance;
  return &instance;
}
...
void mlir::aster::amdgcn::getInstEffects(
  ...
  auto addEffectsForRegister = [&](Type type, MemoryEffects::Effect *effect) {
    ...
    RegisterRange range = regType.getAsRange();
    auto kind = static_cast<RegisterKind>(regType.getRegisterKindAsInt());
    ...
    for (int i = 0; i < size; ++i) {
      ...
      case RegisterKind::SGPR:
        resource = SGPRResource::get(SGPRType::get(ctx, Register(regNum)));
        break;
      case RegisterKind::VGPR:
        resource = VGPRResource::get(VGPRType::get(ctx, Register(regNum)));
        break;
      case RegisterKind::AGPR:
        resource = AGPRResource::get(AGPRType::get(ctx, Register(regNum)));
        break;
      }
...
LogicalResult MakeRegisterRangeOp::inferReturnTypes(
  ...
  auto fTy = cast<RegisterTypeInterface>(operands[0].getType());
  if (llvm::any_of(TypeRange(operands), [&](Type type) {
        auto oTy = cast<RegisterTypeInterface>(type);
  ...
  for (Type type : TypeRange(operands)) {
    int reg =
        cast<RegisterTypeInterface>(type).getAsRange().begin().getRegister();
...
LogicalResult SplitRegisterRangeOp::inferReturnTypes(
  ...
  auto rangeType = cast<RegisterTypeInterface>(inputType);
...
LogicalResult KernelOp::verify() {
  // Verify the body region and its arguments
  Region &bodyRegion = getBodyRegion();

  int32_t numEndKernel = 0;

  // Verify that all EndKernelOp terminators in the kernel have no operands
...
//===----------------------------------------------------------------------===//
// KernelOp CallableOpInterface
//===----------------------------------------------------------------------===//

Region *KernelOp::getCallableRegion() { return &getBodyRegion(); }

ArrayRef<Type> KernelOp::getArgumentTypes() {
  Region &bodyRegion = getBodyRegion();
  if (bodyRegion.empty() || bodyRegion.front().getNumArguments() == 0)
    return {};
  Block &entryBlock = bodyRegion.front();
  // Collect argument types into a vector stored in the operation's context
  // This is safe because the types are derived from the block arguments which
  // persist for the lifetime of the operation
  SmallVector<Type> argTypes;
  argTypes.reserve(entryBlock.getNumArguments());
  for (BlockArgument arg : entryBlock.getArguments())
    argTypes.push_back(arg.getType());
  // Store in a per-operation cache or return empty for now
  // TODO: Implement proper caching if needed
  return {};
}

ArrayRef<Type> KernelOp::getResultTypes() {
  // Kernels don't return values
  return {};
}

//===----------------------------------------------------------------------===//
// KernelOp ArgAndResultAttrsOpInterface
//===----------------------------------------------------------------------===//

ArrayAttr KernelOp::getArgAttrsAttr() {
  return getOperation()->getAttrOfType<ArrayAttr>("arg_attrs");
}

ArrayAttr KernelOp::getResAttrsAttr() {
  return getOperation()->getAttrOfType<ArrayAttr>("res_attrs");
}

void KernelOp::setArgAttrsAttr(ArrayAttr attrs) {
  getOperation()->setAttr("arg_attrs", attrs);
}

void KernelOp::setResAttrsAttr(ArrayAttr attrs) {
  getOperation()->setAttr("res_attrs", attrs);
}

Attribute KernelOp::removeArgAttrsAttr() {
  return getOperation()->removeAttr("arg_attrs");
}

Attribute KernelOp::removeResAttrsAttr() {
  return getOperation()->removeAttr("res_attrs");
}
