...
mlir::aster::amdgcn::getRegKind(RegisterTypeInterface regTy) {
  if (isa<AGPRType, AGPRRangeType, SGPRType, SGPRRangeType, VGPRType,
          VGPRRangeType>(regTy))
    return static_cast<RegisterKind>(regTy.getRegisterKindAsInt());
  return std::nullopt;
}
...
bool mlir::aster::amdgcn::isAMDRegOf(Type type, RegisterKind kind,
  ...
  return rTy.getRegisterKindAsInt() == static_cast<int8_t>(kind) &&
