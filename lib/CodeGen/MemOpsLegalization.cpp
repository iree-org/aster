...
namespace {
/// Helper to convert OpFoldResult to Value, creating a constant if needed.
...
static Value computeByteOffset(OpBuilder &builder, Location loc,
  ...
  // Get offset as Value.
  Value offsetVal =
      getValueOrCreateConstant(builder, loc, descriptor.getOffset());
  affineOperands.push_back(offsetVal);

  // Get strides as Values.
  for (int64_t i = 0; i < rank; ++i) {
    Value strideVal =
        getValueOrCreateConstant(builder, loc, descriptor.getStride(i));
...
struct MemRefDimToValue : public OpConversionPattern<memref::DimOp> {
  ...
  matchAndRewrite(memref::DimOp op, OneToNOpAdaptor adaptor,
    ...
    Value sizeVal = getValueOrCreateConstant(rewriter, loc, size);
...
struct ExtractStridedMetadataToValues
  ...
  matchAndRewrite(memref::ExtractStridedMetadataOp op, OneToNOpAdaptor adaptor,
    ...
    // 2. Offset.
    Value offsetVal =
        getValueOrCreateConstant(rewriter, loc, descriptor.getOffset());
    replacements.push_back({offsetVal});

    // 3. Sizes.
    for (int64_t i = 0; i < rank; ++i) {
      Value sizeVal =
          getValueOrCreateConstant(rewriter, loc, descriptor.getSize(i));
    ...
    for (int64_t i = 0; i < rank; ++i) {
      Value strideVal =
          getValueOrCreateConstant(rewriter, loc, descriptor.getStride(i));
...
struct MemRefLoadToPtr : public OpConversionPattern<memref::LoadOp> {
  ...
  matchAndRewrite(memref::LoadOp op, OneToNOpAdaptor adaptor,
    ...
    ArrayRef<ValueRange> indices = adaptor.getIndices();

    // Flatten indices from ArrayRef<ValueRange> to SmallVector<Value>.
    SmallVector<Value> flatIndices;
    for (auto &ins : indices)
      flatIndices.append(ins.begin(), ins.end());

    // Compute element pointer.
    auto ptrOrFailure = computeElementPtr(rewriter, loc, memrefType,
                                          adaptor.getMemref(), flatIndices);
...
struct VectorLoadToPtr : public OpConversionPattern<vector::LoadOp> {
  ...
  matchAndRewrite(vector::LoadOp op, OneToNOpAdaptor adaptor,
    ...
    // Flatten indices from ArrayRef<ValueRange> to SmallVector<Value>.
    SmallVector<Value> flatIndices;
    for (auto &ins : adaptor.getIndices())
      flatIndices.append(ins.begin(), ins.end());

    // Compute element pointer.
    auto ptrOrFailure = computeElementPtr(rewriter, loc, memrefType,
                                          adaptor.getBase(), flatIndices);
    if (failed(ptrOrFailure))
      return rewriter.notifyMatchFailure(op,
                                         "failed to compute element pointer");
