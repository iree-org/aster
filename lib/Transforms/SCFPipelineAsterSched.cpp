...
///
/// When the wrapped stage gap exceeds 1, a shift of that
...
static LogicalResult analyzeLoop(scf::ForOp originalForOp,
  ...
  // Rotate the static body order so the selected stage's ops lead the kernel
  // body (e.g. MFMA ahead of GL/DR). Stages and info.maxStage are unchanged:
  // each op still computes for the same logical iteration via IV - stage*step,
  // so the runtime schedule is preserved exactly. Rotation only permutes the
  // textual order in which ops appear in prologue / kernel / epilogue, with
  // matching peeling falling out of the unchanged maxStage.
  if (enableRotateStage) {
    bool allOpsHaveInferredStage = llvm::all_of(
        info.opOrder, [&](Operation *op) { return info.stages.count(op); });
    if (!allOpsHaveInferredStage)
      return originalForOp.emitError(
...
void SCFPipelineAsterSchedPass::runOnOperation() {
  ...
  auto walkResult = getOperation()->walk([&](scf::ForOp originalForOp)
    ...
    // iteration (ki = maxStage) has the use-stage processing after all
    // prologue stages have executed.
