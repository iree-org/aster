...
LogicalResult IdDimOpConversion<Op, COp>::matchAndRewrite(
  ...
  Value res =
      COp::create(rewriter, op.getLoc(),
                  static_cast<pir::Dim>(static_cast<int>(op.getDimension())));
...
void ToIntArith::runOnOperation() {
  ...
  target.addLegalDialect<arith::ArithDialect, pir::PIRDialect>();
  target.addDynamicallyLegalOp<pir::AssumeRangeOp>(
  ...
  conversionPatterns.add<IdDimOpConversion<gpu::BlockIdOp, pir::BlockIdOp>,
                         IdDimOpConversion<gpu::BlockDimOp, pir::BlockDimOp>,
                         IdDimOpConversion<gpu::ThreadIdOp, pir::ThreadIdOp>,
                         IdDimOpConversion<gpu::GridDimOp, pir::GridDimOp>,
                         GenericOpConversion<pir::AssumeRangeOp>>(
