  ...
  func.func private @load_global_at_byte_off(!sx4, index) -> !future_global_read
  ...
  func.func private @k_load_a_16x32_from_global(%m_t: index, %k_t: index,
      %A_rsrc: !sx4, %k: index, %stride_AB: index, %m_base: index)
      -> !gfut_a_buf {
    ...
    scf.for %kt = %c0 to %k_t step %c1 {
      ...
      scf.for %i = %c0 to %m_t step %c1 {
        ...
        %fut = func.call @load_global_tile_16x64_b(%A_rsrc, %m_off, %k_offset, %stride_AB)
            {sched.stage = {{STAGE_GLOBAL_LOAD}} : i32}
            : (!sx4, index, index, index) -> !future_global_read
  ...
  func.func private @k_load_b_16x32_from_global(%n_t: index, %k_t: index,
      %B_rsrc: !sx4, %k: index, %stride_AB: index, %n_base: index)
      -> !gfut_b_buf {
    %c0 = arith.constant 0 : index
    %c1 = arith.constant 1 : index
    ...
    scf.for %kt = %c0 to %k_t step %c1 {
      ...
      scf.for %i = %c0 to %n_t step %c1 {
        ...
        %fut = func.call @load_global_tile_16x64_b(%B_rsrc, %n_off, %k_offset, %stride_AB)
            {sched.stage = {{STAGE_GLOBAL_LOAD}} : i32}
            : (!sx4, index, index, index) -> !future_global_read
  ...
  func.func private @k_issue_global_loads_a(%addrs: memref<?xindex>,
    ...
    scf.for %idx = %c0 to %buf_size step %c1 {
      ...
      %fut = func.call @load_global_at_byte_off(%A_rsrc, %byte_off)
          {sched.stage = {{STAGE_GLOBAL_LOAD}} : i32}
  ...
  func.func private @k_issue_global_loads_b(%addrs: memref<?xindex>,
    ...
    scf.for %idx = %c0 to %buf_size step %c1 {
      %byte_off = memref.load %addrs[%idx] : memref<?xindex>
      %fut = func.call @load_global_at_byte_off(%B_rsrc, %byte_off)
          {sched.stage = {{STAGE_GLOBAL_LOAD}} : i32}
          : (!sx4, index) -> !future_global_read
      memref.store %fut, %gfut_b[%idx] : !gfut_b_buf
    } {aster.constexpr}
  ...
  func.func private @store_c_tiles(%m_t: index, %n_t: index,
      %c_buf: !c_buf, %C_rsrc: !sx4, %stride_C: index,
    ...
    scf.for %mt = %c0 to %m_t step %c1 {
      scf.for %nt = %c0 to %n_t step %c1 {
        ...
        func.call @store_global_C_mfma_f32_16x16x16_f16(%c_tile, %C_rsrc, %m_off, %n_off, %stride_C)
            : (!rt_C_f32, !sx4, index, index, index) -> ()
      } {aster.constexpr}
    } {aster.constexpr}
    return
  }
