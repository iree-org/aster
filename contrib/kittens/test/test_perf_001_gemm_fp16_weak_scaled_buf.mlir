...
amdgcn.module @kittens_gemm_f16_weak_scaled target = #amdgcn.target<gfx942> isa = #amdgcn.isa<cdna3> {
  ...
  func.func private @store_global_C_mfma_f32_16x16x16_f16(!rt_C_f32, !sx4, index, index, index)
  // From global_16x64_b.mlir / lds_16x64_b.mlir (dwordx4 global load + 16x32 LDS operations)
  func.func private @load_global_tile_16x64_b(!sx4, index, index, index) -> !future_global_read
...
{{K_LOOP_HELPERS}}
  ...
  amdgcn.kernel @gemm_f16_weak_scaled arguments <[
    ...
    // Construct buffer resource descriptors (SRDs) from pointers
    %A_rsrc = func.call @make_raw_buffer_rsrc(%A_ptr) : (!sx2) -> !sx4
    %B_rsrc = func.call @make_raw_buffer_rsrc(%B_ptr) : (!sx2) -> !sx4
    %C_rsrc = func.call @make_raw_buffer_rsrc(%C_ptr) : (!sx2) -> !sx4
    ...
    func.call @store_c_tiles(%c_M_T, %c_N_T, %C_buf, %C_rsrc, %stride_C, %m_base, %n_base)
        : (index, index, !c_buf, !sx4, index, index, index) -> ()
