...
class TestKittensGEMMLDS1Buffer_AGPR:
    ...
    @pytest.mark.parametrize("k", [32, 64])
    def test_gemm_lds_1buf_hoist_wait(self, k):
        """Same as test_gemm_lds_1buf but with hoist-iter-arg-waits enabled."""
        k_tiles = k // 32
        stride_ab = k * 2
    ...
    @pytest.mark.parametrize("k", [32, 64])
    def test_gemm_lds_1buf_ll_sched_hoist_wait(self, k):
