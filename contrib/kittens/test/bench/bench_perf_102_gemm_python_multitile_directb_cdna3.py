"""Benchmark: Weak-scaling TFLOPS sweep for Python multi-tile GEMM (test_102, direct_b pinned, CDNA3).

Single config (repro):
    python .../bench_perf_102_..._directb_cdna3 m4864xn4096xk8192_wg38x32x1_w2x2x1_twg8x8x1_...

Sweep (default M=N=K=4096):
    python .../bench_perf_102_..._directb_cdna3 --compile-sample 100

Pin individual dimensions (others default to 4096):
    python .../bench_perf_102_..._directb_cdna3 --compile-sample 500 --m 2432 --k 128

Pin all three at once (exclusive with --m/--n/--k):
    python .../bench_perf_102_..._directb_cdna3 --compile-sample 500 --size 2432x12288x4096
"""
...
from test_102_gemm_python_multitile_directb_cdna3 import (
...
def _repro_cmd(cfg):
    return f"python contrib/kittens/test/bench/bench_perf_102_gemm_python_multitile_directb_cdna3.py {cfg.label}"
