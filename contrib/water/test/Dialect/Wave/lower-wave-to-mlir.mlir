// RUN: water-opt %s -allow-unregistered-dialect -lower-wave-to-mlir -water-wave-lower-transform-payload --mlir-print-local-scope --split-input-file --verify-diagnostics | FileCheck %s
...
// CHECK: module {
// CHECK-NOT: transform.payload
transform.payload attributes {normal_forms = [#wave.normal_form<full_types,index_exprs,memory_only_types,resolved_allocations,ordered_syms>]} {
  builtin.module {
    // CHECK-LABEL: func.func @test_transform.payload_module_lowered
    func.func @test_transform.payload_module_lowered() attributes {wave.hyperparameters = #wave.hyperparameters<{}>} {
      return
    }
  }
}
