  ...
  //===--------------------------------------------------------------------===//
  // 9. Fused LDS read + MFMA with intra-step prefetch (2D wave layout, 32x32)
  //===--------------------------------------------------------------------===//
  ...
  // eliminates the lgkmcnt staircase stall in the unprefetched version.
  // k_inner = k_t * 4 (four 32x32x8 MFMA K-steps per K_T chunk of 32 f16).
  // tile_off = base + (kt * m_t_ld + m_abs * 2) * 1024 (upper half of 32-row tile).
  // Replaces k_read_lds_a_2d_32x32 + k_read_lds_b_2d_32x32 +
  // k_wait_and_compute_mfmas_2d_32x32.
  func.func private @k_fused_lds_read_compute_2d_32x32(
    ...
    // Prologue: issue reads for ki=0 (kt=0, kh=0, k_byte_offset=0).
    scf.for %m = %c0 to %m_t_per_wave step %c1 {
      %m_abs = affine.apply affine_map<(m)[wb] -> (wb + m)>(%m)[%wm_base]
      %tile_off = affine.apply affine_map<(mabs)[base] -> (base + mabs * 2 * 1024)>
          (%m_abs)[%base_a]
      %fut = func.call @load_lds_A_swizzled_32x32(%tile_off, %c0, %c2)
          {sched.stage = {{STAGE_DS_READ}} : i32}
          : (index, index, index) -> !future_lds_read
    ...
    scf.for %n = %c0 to %n_t_per_wave step %c1 {
      %n_abs = affine.apply affine_map<(n)[wb] -> (wb + n)>(%n)[%wn_base]
      %tile_off = affine.apply affine_map<(nabs)[base] -> (base + nabs * 2 * 1024)>
          (%n_abs)[%base_b]
      %fut = func.call @load_lds_B_swizzled_32x32(%tile_off, %c0, %c2)
          {sched.stage = {{STAGE_DS_READ}} : i32}
          : (index, index, index) -> !future_lds_read
    ...
    scf.for %ki = %c0 to %k_inner step %c1 {
      ...
      scf.if %not_last {
        %kt_next, %kh_next = affine.delinearize_index %ki1 into (%k_t, %c4)
            : index, index
        %k_byte_next = affine.apply affine_map<(kh) -> (kh * 16)>(%kh_next)
        scf.for %m = %c0 to %m_t_per_wave step %c1 {
          %m_abs = affine.apply affine_map<(m)[wb] -> (wb + m)>(%m)[%wm_base]
          %tile_off = affine.apply
              affine_map<(kt, mabs)[base, mtld] -> (base + (kt * mtld + mabs * 2) * 1024)>
              (%kt_next, %m_abs)[%base_a, %m_t_ld]
          %fut = func.call @load_lds_A_swizzled_32x32(%tile_off, %k_byte_next, %c2)
              {sched.stage = {{STAGE_DS_READ}} : i32}
              : (index, index, index) -> !future_lds_read
        ...
        scf.for %n = %c0 to %n_t_per_wave step %c1 {
          %n_abs = affine.apply affine_map<(n)[wb] -> (wb + n)>(%n)[%wn_base]
          %tile_off = affine.apply
              affine_map<(kt, nabs)[base, ntld] -> (base + (kt * ntld + nabs * 2) * 1024)>
              (%kt_next, %n_abs)[%base_b, %n_t_ld]
          %fut = func.call @load_lds_B_swizzled_32x32(%tile_off, %k_byte_next, %c2)
              {sched.stage = {{STAGE_DS_READ}} : i32}
              : (index, index, index) -> !future_lds_read
      ...
      scf.for %m = %c0 to %m_t_per_wave step %c1 {
        ...
        scf.for %n = %c0 to %n_t_per_wave step %c1 {
          ...
          %c_idx = affine.apply affine_map<(m, n)[ntpw] -> (m * ntpw + n)>(%m, %n)[%n_t_per_wave]
          %c_old = memref.load %c_buf[%c_idx] : !c_buf_32
          %c_new = func.call @mfma_f32_32x32x8_f16(%a_val, %b_val, %c_old)
              {sched.stage = {{STAGE_COMPUTE}} : i32, sched.rotate_head}
              : (!rt_A_f16_32, !rt_B_f16_32, !rt_C_f32_32) -> !rt_C_f32_32
          memref.store %c_new, %c_buf[%c_idx] : !c_buf_32
        } {aster.constexpr}
