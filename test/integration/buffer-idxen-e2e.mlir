...
amdgcn.module @buffer_idxen_mod target = #amdgcn.target<gfx942> {
  ...
  func.func private @load_kernargs()
    ...
    %num_records, %t0 = amdgcn.s_load_dword dest %nrec_dest addr %params_ptr offset c(%c0) : outs(!amdgcn.sgpr) ins(!amdgcn.sgpr<[? + 2]>) mods(i32) -> !amdgcn.read_token<constant>

    %soff_dest = amdgcn.alloca : !amdgcn.sgpr
    %soffset, %t1 = amdgcn.s_load_dword dest %soff_dest addr %params_ptr offset c(%c4) : outs(!amdgcn.sgpr) ins(!amdgcn.sgpr<[? + 2]>) mods(i32) -> !amdgcn.read_token<constant>
  ...
  amdgcn.kernel @buffer_idxen_copy_kernel arguments <[
    ...
    %loaded, %tok_ld = amdgcn.buffer_load_dword dest %load_dest addr %src_rsrc offset u(%soffset) + off_idx(%vindex) + c(%c0) {idxen} : outs(!amdgcn.vgpr) ins(!amdgcn.sgpr<[? + 4]>, !amdgcn.sgpr, !amdgcn.vgpr<0>) mods(i32) -> !amdgcn.read_token<flat>

    amdgcn.s_waitcnt vmcnt = 0

    %tok_st = amdgcn.buffer_store_dword data %loaded addr %dst_rsrc offset u(%soffset) + off_idx(%vindex) + c(%c0) {idxen} : ins(!amdgcn.vgpr, !amdgcn.sgpr<[? + 4]>, !amdgcn.sgpr, !amdgcn.vgpr<0>) mods(i32) -> !amdgcn.write_token<flat>
