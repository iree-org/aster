...
amdgcn.module @mfma_cdna4_doubled_k_mod target = #amdgcn.target<gfx950> {
  ...
  func.func private @store_x4(
      ...
      %const_offset: i32) {
    %tok = amdgcn.global_store_dwordx4 ins(%data, %base, offset = %thread_offset) args(%const_offset)
  ...
  amdgcn.kernel @mfma_16x16x32_f16_ones arguments <[
    ...
    %tok = amdgcn.global_store_dwordx4 ins(%result, %c_ptr, offset = %thread_offset) args(%c0_store)
        : ins(!amdgcn.vgpr<[? + 4]>, !amdgcn.sgpr<[? + 2]>, offset = !amdgcn.vgpr) args(i32) -> !amdgcn.write_token<flat>
