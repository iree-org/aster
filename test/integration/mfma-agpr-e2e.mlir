...
amdgcn.module @agpr_mfma_mod target = #amdgcn.target<gfx942> {
  ...
  amdgcn.kernel @agpr_mfma_kernel arguments <[
    ...
    // LDS load A
    %loaded_a_lds, %tok_lds_a = amdgcn.ds_read_b64 outs(%a_reg_range) ins(%thread_offset_f16) args(%c0_i32)
        : outs(!amdgcn.vgpr<[? + 2]>) ins(!amdgcn.vgpr) args(i32) -> !amdgcn.read_token<shared>

    // LDS load B
    %loaded_b_lds, %tok_lds_b = amdgcn.ds_read_b64 outs(%b_reg_range) ins(%thread_offset_f16) args(%c512_i32)
        : outs(!amdgcn.vgpr<[? + 2]>) ins(!amdgcn.vgpr) args(i32) -> !amdgcn.read_token<shared>
