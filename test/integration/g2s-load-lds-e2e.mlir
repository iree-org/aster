...
amdgcn.module @g2s_e2e_mod target = #amdgcn.target<gfx950> {
  ...
  func.func private @load_kernargs()
    ...
    %num_bytes, %t0 = amdgcn.s_load_dword outs(%nbytes_dest) ins(%params_ptr) args(%c0)
  ...
  amdgcn.kernel @g2s_roundtrip_kernel arguments <[
    ...
    %tok_g2s = amdgcn.buffer_load_lds_dword ins(%src_rsrc, off_or_idx = %voffset, %soffset, %m0) args(%c0) {offen}
    ...
    %lds_val, %tok_lds = amdgcn.ds_read_b32 outs(%lds_dest) ins(%voffset) args(%c44)
    ...
    %tok_st = amdgcn.global_store_dword ins(%lds_val, %dst_ptr, offset = %voffset) args(%c0)
