// RUN: aster-opt %s \
// RUN:   --amdgcn-preload-library="library-paths=%p/../../mlir_kernels/library/common/register-init.mlir" \
// RUN:   --inline \
// RUN: | aster-opt \
// RUN:   --pass-pipeline="builtin.module(amdgcn.module(amdgcn.kernel(aster-amdgcn-expand-md-ops)))" \
// RUN: | aster-opt \
// RUN:   --amdgcn-reg-alloc --symbol-dce \
// RUN: | FileCheck %s
//
// RUN: aster-opt %s \
// RUN:   --amdgcn-preload-library="library-paths=%p/../../mlir_kernels/library/common/register-init.mlir" \
// RUN:   --inline \
// RUN: | aster-opt \
// RUN:   --pass-pipeline="builtin.module(amdgcn.module(amdgcn.kernel(aster-amdgcn-expand-md-ops)))" \
// RUN: | aster-opt \
// RUN:   --amdgcn-reg-alloc --symbol-dce | aster-translate --mlir-to-asm \
// RUN: | FileCheck %s --check-prefix=ASM

// CHECK-LABEL: amdgcn.module
//   CHECK-NOT:   load_kernarg_pointers

// ASM-LABEL: compute_kernel:
//       ASM:   s_load_dwordx2 s[2:3], s[0:1], 0
amdgcn.module @mod target = #amdgcn.target<gfx942> {

  // From register-init.mlir (resolved by --amdgcn-preload-library)
  func.func private @alloc_vgprx2() -> (!amdgcn.vgpr<[? + 2]>)
  func.func private @init_vgprx4(%cst: i32) -> (!amdgcn.vgpr<[? + 4]>)

  func.func private @load_kernarg_pointers() -> (!amdgcn.sgpr<[? + 2]>, !amdgcn.sgpr<[? + 2]>, !amdgcn.sgpr<[? + 2]>) {
    // Load kernarg arguments using load_arg operation
    %a_ptr = amdgcn.load_arg 0 : !amdgcn.sgpr<[? + 2]>
    %b_ptr = amdgcn.load_arg 1 : !amdgcn.sgpr<[? + 2]>
    %c_ptr = amdgcn.load_arg 2 : !amdgcn.sgpr<[? + 2]>

    amdgcn.s_waitcnt lgkmcnt = 0

    return %a_ptr, %b_ptr, %c_ptr : !amdgcn.sgpr<[? + 2]>, !amdgcn.sgpr<[? + 2]>, !amdgcn.sgpr<[? + 2]>
  }

  amdgcn.kernel @compute_kernel arguments <[
    #amdgcn.buffer_arg<address_space = generic, access = read_only>,
    #amdgcn.buffer_arg<address_space = generic, access = read_only>,
    #amdgcn.buffer_arg<address_space = generic, access = read_write>
  ]> attributes {shared_memory_size = 1024 : i32} {

    // Load kernarg pointers
    %a_ptr, %b_ptr, %c_ptr = func.call @load_kernarg_pointers()
      : () -> (!amdgcn.sgpr<[? + 2]>, !amdgcn.sgpr<[? + 2]>, !amdgcn.sgpr<[? + 2]>)

    // v0 reserved for threadidx.x
    %threadidx_x = amdgcn.alloca : !amdgcn.vgpr<0>

    // Allocate register ranges via library functions
    %a_reg_range = func.call @alloc_vgprx2() : () -> (!amdgcn.vgpr<[? + 2]>)
    %b_reg_range = func.call @alloc_vgprx2() : () -> (!amdgcn.vgpr<[? + 2]>)
    %c0 = arith.constant 0 : i32
    %c_reg_range = func.call @init_vgprx4(%c0) : (i32) -> (!amdgcn.vgpr<[? + 4]>)

    // global_load (A)
    %offset_a = amdgcn.alloca : !amdgcn.vgpr
    %c3 = arith.constant 3 : i32 // shift left by dwordx2 size (8 == 2 << 3).
    %thread_offset_f16 = amdgcn.v_lshlrev_b32 outs(%offset_a) ins(%c3, %threadidx_x) : outs(!amdgcn.vgpr) ins(i32, !amdgcn.vgpr<0>)
    %c0_i32 = arith.constant 0 : i32
    %c512_i32 = arith.constant 512 : i32

    %loaded_a, %tok_load_a = amdgcn.global_load_dwordx2 outs(%a_reg_range) ins(%a_ptr, offset = %thread_offset_f16) args(%c0_i32)
        : outs(!amdgcn.vgpr<[? + 2]>) ins(!amdgcn.sgpr<[? + 2]>, offset = !amdgcn.vgpr) args(i32) -> !amdgcn.read_token<flat>

    // global_load (B)
    %loaded_b, %tok_load_b = amdgcn.global_load_dwordx2 outs(%b_reg_range) ins(%b_ptr, offset = %thread_offset_f16) args(%c0_i32)
        : outs(!amdgcn.vgpr<[? + 2]>) ins(!amdgcn.sgpr<[? + 2]>, offset = !amdgcn.vgpr) args(i32) -> !amdgcn.read_token<flat>

    // s_waitcnt(vmcnt(0))
    amdgcn.s_waitcnt vmcnt = 0

    // // ds_store to ldsA
    %tok_ds_a = amdgcn.ds_write_b64 ins(%thread_offset_f16, %loaded_a) args(%c0_i32)
        : ins(!amdgcn.vgpr, !amdgcn.vgpr<[? + 2]>) args(i32) -> !amdgcn.write_token<shared>

    // ds_store to ldsB
    %tok_ds_b = amdgcn.ds_write_b64 ins(%thread_offset_f16, %loaded_b) args(%c512_i32)
        : ins(!amdgcn.vgpr, !amdgcn.vgpr<[? + 2]>) args(i32) -> !amdgcn.write_token<shared>

    // s_waitcnt(lgkmcnt(0))
    amdgcn.s_waitcnt lgkmcnt = 0

    // ds_load from ldsA
    %loaded_a_from_lds, %tok_lds_a = amdgcn.ds_read_b64 outs(%a_reg_range) ins(%thread_offset_f16) args(%c0_i32)
        : outs(!amdgcn.vgpr<[? + 2]>) ins(!amdgcn.vgpr) args(i32) -> !amdgcn.read_token<shared>

    // ds_load from ldsB
    %loaded_b_from_lds, %tok_lds_b = amdgcn.ds_read_b64 outs(%b_reg_range) ins(%thread_offset_f16) args(%c512_i32)
        : outs(!amdgcn.vgpr<[? + 2]>) ins(!amdgcn.vgpr) args(i32) -> !amdgcn.read_token<shared>

    // s_waitcnt(lgkmcnt(0))
    amdgcn.s_waitcnt lgkmcnt = 0

    // mfma - A and B need 2 VGPRs each, C needs 4 VGPRs
    %c_mfma_result = amdgcn.vop3p.vop3p_mai #amdgcn.inst<v_mfma_f32_16x16x16_f16> %c_reg_range, %loaded_a_from_lds, %loaded_b_from_lds, %c_reg_range
      : <[? + 2]>, <[? + 2]>, !amdgcn.vgpr<[? + 4]> -> !amdgcn.vgpr<[? + 4]>

    // global_store of c_mfma_result
    %c4 = arith.constant 4 : i32 // shift left by dwordx4 size (16 == 2 << 4).
    %thread_offset_f32 = amdgcn.v_lshlrev_b32 outs(%offset_a) ins(%c4, %threadidx_x) : outs(!amdgcn.vgpr) ins(i32, !amdgcn.vgpr<0>)
    %tok_store_c = amdgcn.global_store_dwordx4 ins(%c_mfma_result, %c_ptr, offset = %thread_offset_f32) args(%c0_i32)
        : ins(!amdgcn.vgpr<[? + 4]>, !amdgcn.sgpr<[? + 2]>, offset = !amdgcn.vgpr) args(i32) -> !amdgcn.write_token<flat>

    // s_waitcnt(vmcnt(0))
    amdgcn.s_waitcnt vmcnt = 0

    amdgcn.end_kernel
  }
}
