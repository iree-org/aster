...
amdgcn.module @mod target = #amdgcn.target<gfx942> {
  ...
  amdgcn.kernel @compute_kernel arguments <[
    ...
    // ds_load from ldsA
    %loaded_a_from_lds, %tok_lds_a = amdgcn.ds_read_b64 dest %a_reg_range addr %thread_offset_f16 offset c(%c0_i32) : outs(!amdgcn.vgpr<[? + 2]>) ins(!amdgcn.vgpr) mods(i32) -> !amdgcn.read_token<shared>

    // ds_load from ldsB
    %loaded_b_from_lds, %tok_lds_b = amdgcn.ds_read_b64 dest %b_reg_range addr %thread_offset_f16 offset c(%c512_i32) : outs(!amdgcn.vgpr<[? + 2]>) ins(!amdgcn.vgpr) mods(i32) -> !amdgcn.read_token<shared>

    // s_waitcnt(lgkmcnt(0))
    amdgcn.s_waitcnt lgkmcnt = 0

    // mfma - A and B need 2 VGPRs each, C needs 4 VGPRs
    %c_mfma_result = amdgcn.v_mfma_f32_16x16x16_f16 outs(%c_reg_range) ins(%loaded_a_from_lds, %loaded_b_from_lds, %c_reg_range)
    : outs(!amdgcn.vgpr<[? + 4]>)
    ...
    %tok_store_c = amdgcn.global_store_dwordx4 data %c_mfma_result addr %c_ptr offset d(%thread_offset_f32) + c(%c0_i32) : ins(!amdgcn.vgpr<[? + 4]>, !amdgcn.sgpr<[? + 2]>, !amdgcn.vgpr) mods(i32) -> !amdgcn.write_token<flat>
