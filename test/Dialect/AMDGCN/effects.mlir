...
// CHECK-NEXT:    %{{.*}} = global_load_dword outs(%[[R13]]) ins(%[[R11]]) args(%{{.*}}) : outs(!amdgcn.vgpr<12>) ins(!amdgcn.vgpr<[10 : 12]>) args(i32) -> !amdgcn.read_token<flat>
// CHECK-NEXT:    %{{.*}} = s_load_dwordx2 outs(%{{.*}}) ins(%{{.*}}) args(%{{.*}}) : outs(!amdgcn.sgpr<[2 : 4]>) ins(!amdgcn.sgpr<[0 : 2]>) args(i32) -> !amdgcn.read_token<constant>
// CHECK-NEXT:    s_waitcnt vmcnt = 0
//
// We want to make sure R7, R8 and R11 are not reused after CSE: a proper liveness analysis is needed.
// CHECK-NEXT:    v_mov_b32 outs(%[[R7]]) ins(%{{.*}}) : outs(!amdgcn.vgpr<10>) ins(!amdgcn.sgpr<2>)
// CHECK-NEXT:    v_mov_b32 outs(%[[R8]]) ins(%{{.*}}) : outs(!amdgcn.vgpr<11>) ins(!amdgcn.sgpr<3>)
// CHECK-NEXT:    %{{.*}} = global_store_dword ins(%[[R13]], %[[R11]]) args(%{{.*}}) : ins(!amdgcn.vgpr<12>, !amdgcn.vgpr<[10 : 12]>) args(i32) -> !amdgcn.write_token<flat>
// CHECK-NEXT:    end_kernel
  kernel @gpu_copy_kernel_unchecked {
    ...
    %t1 = amdgcn.s_load_dwordx2 outs(%5) ins(%2) args(%c0_i32_mig1) : outs(!amdgcn.sgpr<[2 : 4]>) ins(!amdgcn.sgpr<[0 : 2]>) args(i32) -> !amdgcn.read_token<constant>
    ...
    %t2 = amdgcn.global_load_dword outs(%13) ins(%11) args(%c0_i32_mig1)
        : outs(!amdgcn.vgpr<12>) ins(!amdgcn.vgpr<[10 : 12]>) args(i32) -> !amdgcn.read_token<flat>
    ...
    %t3 = amdgcn.s_load_dwordx2 outs(%20) ins(%17) args(%c0_i32_mig1) : outs(!amdgcn.sgpr<[2 : 4]>) ins(!amdgcn.sgpr<[0 : 2]>) args(i32) -> !amdgcn.read_token<constant>
    amdgcn.s_waitcnt vmcnt = 0
    ...
    %t4 = amdgcn.global_store_dword ins(%13, %26) args(%c0_i32_mig1)
        : ins(!amdgcn.vgpr<12>, !amdgcn.vgpr<[10 : 12]>) args(i32) -> !amdgcn.write_token<flat>
    end_kernel
  }
}
