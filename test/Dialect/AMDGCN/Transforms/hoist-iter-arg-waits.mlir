...
func.func @hoist_pure_iter_arg_wait(%addr: !amdgcn.vgpr<[? + 2]>) {
  ...
  %pro_data, %pro_tok = amdgcn.global_load_dword outs(%dest) ins(%addr) args(%c0_i32_mig1)
  ...
  %res:2 = scf.for %i = %c1 to %c4 step %c1
    ...
    %new_data, %new_tok = amdgcn.global_load_dword outs(%dest) ins(%addr) args(%c0_i32_mig2)
        : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr<[? + 2]>) args(i32) -> !amdgcn.read_token<flat>
...
func.func @clone_mixed_dep_wait(%addr: !amdgcn.vgpr<[? + 2]>) {
  ...
  %pro_data, %pro_tok = amdgcn.global_load_dword outs(%dest) ins(%addr) args(%c0_i32_mig3)
      : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr<[? + 2]>) args(i32) -> !amdgcn.read_token<flat>
  %res:2 = scf.for %i = %c1 to %c4 step %c1
      iter_args(%iter_tok = %pro_tok, %iter_data = %pro_data)
      -> (!amdgcn.read_token<flat>, !amdgcn.vgpr) {
    %c0_i32_mig4 = arith.constant 0 : i32
    %new_data, %new_tok = amdgcn.global_load_dword outs(%dest2) ins(%addr) args(%c0_i32_mig4)
        : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr<[? + 2]>) args(i32) -> !amdgcn.read_token<flat>
...
func.func @hoist_multiple_waits(%data_in: !amdgcn.vgpr, %lds_addr_a: !amdgcn.vgpr, %lds_addr_b: !amdgcn.vgpr) {
  ...
  %wtok_a = amdgcn.ds_write_b32 ins(%lds_addr_a, %data_in) args(%c0_i32)
      : ins(!amdgcn.vgpr, !amdgcn.vgpr) args(i32) -> !amdgcn.write_token<shared>
  %wtok_b = amdgcn.ds_write_b32 ins(%lds_addr_b, %data_in) args(%c0_i32)
      : ins(!amdgcn.vgpr, !amdgcn.vgpr) args(i32) -> !amdgcn.write_token<shared>
  %res:2 = scf.for %i = %c1 to %c6 step %c1
    ...
    %new_wtok_a = amdgcn.ds_write_b32 ins(%lds_addr_a, %data_in) args(%c0_i32)
        : ins(!amdgcn.vgpr, !amdgcn.vgpr) args(i32) -> !amdgcn.write_token<shared>
    amdgcn.wait deps %iter_wtok_a : !amdgcn.write_token<shared>
    %c0_i32_mig1 = arith.constant 0 : i32
    %read_data, %rtok = amdgcn.ds_read_b32 outs(%s_read) ins(%lds_addr_a) args(%c0_i32_mig1)
        : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr) args(i32) -> !amdgcn.read_token<shared>
    %new_wtok_b = amdgcn.ds_write_b32 ins(%lds_addr_b, %data_in) args(%c0_i32)
        : ins(!amdgcn.vgpr, !amdgcn.vgpr) args(i32) -> !amdgcn.write_token<shared>
...
func.func @no_hoist_intra_only_wait(%addr: !amdgcn.vgpr<[? + 2]>) {
  ...
  %pro_data, %pro_tok = amdgcn.global_load_dword outs(%dest) ins(%addr) args(%c0_i32_mig5)
      : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr<[? + 2]>) args(i32) -> !amdgcn.read_token<flat>
  %res:2 = scf.for %i = %c1 to %c4 step %c1
      iter_args(%iter_tok = %pro_tok, %iter_data = %pro_data)
      -> (!amdgcn.read_token<flat>, !amdgcn.vgpr) {
    ...
    %new_data, %new_tok = amdgcn.global_load_dword outs(%dest) ins(%addr) args(%c0_i32_mig6)
        : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr<[? + 2]>) args(i32) -> !amdgcn.read_token<flat>
...
func.func @no_hoist_across_nested_loops(%addr: !amdgcn.vgpr<[? + 2]>, %lds_addr: !amdgcn.vgpr) {
  ...
  %pro_data, %pro_tok = amdgcn.global_load_dword outs(%dest) ins(%addr) args(%c0_i32_mig7)
      : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr<[? + 2]>) args(i32) -> !amdgcn.read_token<flat>
  ...
  %res:2 = scf.for %i = %c1 to %c4 step %c1
    ...
    %new_data, %new_tok = amdgcn.global_load_dword outs(%dest) ins(%addr) args(%c0_i32_mig8)
        : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr<[? + 2]>) args(i32) -> !amdgcn.read_token<flat>
    amdgcn.wait deps %outer_tok : !amdgcn.read_token<flat>
    // Inner loop with its own iter_arg token
    %init_wtok = amdgcn.ds_write_b32 ins(%lds_addr, %outer_data) args(%c0_i32)
        : ins(!amdgcn.vgpr, !amdgcn.vgpr) args(i32) -> !amdgcn.write_token<shared>
    %inner_res = scf.for %j = %c0 to %c4 step %c1
      ...
      %new_inner_wtok = amdgcn.ds_write_b32 ins(%lds_addr, %outer_data) args(%c0_i32)
          : ins(!amdgcn.vgpr, !amdgcn.vgpr) args(i32) -> !amdgcn.write_token<shared>
...
func.func @hoist_barrier_after_waits(%data_in: !amdgcn.vgpr, %lds_addr: !amdgcn.vgpr) {
  ...
  %c0_i32 = arith.constant 0 : i32
  %s_read = amdgcn.alloca : !amdgcn.vgpr
  %init_wtok = amdgcn.ds_write_b32 ins(%lds_addr, %data_in) args(%c0_i32)
      : ins(!amdgcn.vgpr, !amdgcn.vgpr) args(i32) -> !amdgcn.write_token<shared>
  %c0_i32_mig2 = arith.constant 0 : i32
  %init_data, %init_rtok = amdgcn.ds_read_b32 outs(%s_read) ins(%lds_addr) args(%c0_i32_mig2)
      : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr) args(i32) -> !amdgcn.read_token<shared>
  %res:2 = scf.for %i = %c1 to %c4 step %c1
    ...
    amdgcn.s_barrier
    %c0_i32_mig3 = arith.constant 0 : i32
    %read_data, %rtok = amdgcn.ds_read_b32 outs(%s_read) ins(%lds_addr) args(%c0_i32_mig3)
        : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr) args(i32) -> !amdgcn.read_token<shared>
    %new_wtok = amdgcn.ds_write_b32 ins(%lds_addr, %data_in) args(%c0_i32)
        : ins(!amdgcn.vgpr, !amdgcn.vgpr) args(i32) -> !amdgcn.write_token<shared>
...
func.func @no_move_barrier_with_wait_after(%data_in: !amdgcn.vgpr, %lds_addr: !amdgcn.vgpr) {
  ...
  %init_wtok = amdgcn.ds_write_b32 ins(%lds_addr, %data_in) args(%c0_i32)
      : ins(!amdgcn.vgpr, !amdgcn.vgpr) args(i32) -> !amdgcn.write_token<shared>
  %c0_i32_mig4 = arith.constant 0 : i32
  %init_data, %init_rtok = amdgcn.ds_read_b32 outs(%s_read) ins(%lds_addr) args(%c0_i32_mig4)
      : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr) args(i32) -> !amdgcn.read_token<shared>
  %res:2 = scf.for %i = %c1 to %c4 step %c1
      iter_args(%iter_wtok = %init_wtok, %iter_data = %init_data)
      -> (!amdgcn.write_token<shared>, !amdgcn.vgpr) {
    ...
    amdgcn.s_barrier
    %c0_i32_mig5 = arith.constant 0 : i32
    %read_data, %rtok = amdgcn.ds_read_b32 outs(%s_read) ins(%lds_addr) args(%c0_i32_mig5)
        : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr) args(i32) -> !amdgcn.read_token<shared>
...
func.func @no_hoist_wait_inside_scf_if(%addr: !amdgcn.vgpr<[? + 2]>, %cond: i1) {
  ...
  %pro_data, %pro_tok = amdgcn.global_load_dword outs(%dest) ins(%addr) args(%c0_i32_mig9)
      : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr<[? + 2]>) args(i32) -> !amdgcn.read_token<flat>
  %res:2 = scf.for %i = %c1 to %c4 step %c1
      iter_args(%iter_tok = %pro_tok, %iter_data = %pro_data)
      -> (!amdgcn.read_token<flat>, !amdgcn.vgpr) {
    %c0_i32_mig10 = arith.constant 0 : i32
    %new_data, %new_tok = amdgcn.global_load_dword outs(%dest) ins(%addr) args(%c0_i32_mig10)
        : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr<[? + 2]>) args(i32) -> !amdgcn.read_token<flat>
...
func.func @no_hoist_count_only_wait(%addr: !amdgcn.vgpr<[? + 2]>) {
  ...
  %pro_data, %pro_tok = amdgcn.global_load_dword outs(%dest) ins(%addr) args(%c0_i32_mig11)
      : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr<[? + 2]>) args(i32) -> !amdgcn.read_token<flat>
  %res:2 = scf.for %i = %c1 to %c4 step %c1
      iter_args(%iter_tok = %pro_tok, %iter_data = %pro_data)
      -> (!amdgcn.read_token<flat>, !amdgcn.vgpr) {
    %c0_i32_mig12 = arith.constant 0 : i32
    %new_data, %new_tok = amdgcn.global_load_dword outs(%dest) ins(%addr) args(%c0_i32_mig12)
        : outs(!amdgcn.vgpr) ins(!amdgcn.vgpr<[? + 2]>) args(i32) -> !amdgcn.read_token<flat>
