...
// HOIST-NEXT:    amdgcn.load global_load_dword
...
func.func @hoist_pure_iter_arg_wait(%addr: !amdgcn.vgpr<[? + 2]>) {
  ...
  %pro_data, %pro_tok = amdgcn.load global_load_dword dest %dest addr %addr
  ...
  %res:2 = scf.for %i = %c1 to %c4 step %c1
    ...
    %new_data, %new_tok = amdgcn.load global_load_dword dest %dest addr %addr
      : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr<[? + 2]>) -> !amdgcn.read_token<flat>
...
// HOIST-NEXT:    %[[ND:.*]], %[[NT:.*]] = amdgcn.load global_load_dword
...
func.func @clone_mixed_dep_wait(%addr: !amdgcn.vgpr<[? + 2]>) {
  ...
  %res:2 = scf.for %i = %c1 to %c4 step %c1
      iter_args(%iter_tok = %pro_tok, %iter_data = %pro_data)
      -> (!amdgcn.read_token<flat>, !amdgcn.vgpr) {
    %new_data, %new_tok = amdgcn.load global_load_dword dest %dest2 addr %addr
      : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr<[? + 2]>) -> !amdgcn.read_token<flat>
...
// MERGE:         amdgcn.store ds_write_b32
// MERGE:         amdgcn.load ds_read_b32
...
func.func @hoist_multiple_waits(%data_in: !amdgcn.vgpr, %lds_addr_a: !amdgcn.vgpr, %lds_addr_b: !amdgcn.vgpr) {
  ...
  %wtok_a = amdgcn.store ds_write_b32 data %data_in addr %lds_addr_a offset c(%c0_i32)
    : ins(!amdgcn.vgpr, !amdgcn.vgpr, i32) -> !amdgcn.write_token<shared>
  %wtok_b = amdgcn.store ds_write_b32 data %data_in addr %lds_addr_b offset c(%c0_i32)
    : ins(!amdgcn.vgpr, !amdgcn.vgpr, i32) -> !amdgcn.write_token<shared>
  %res:2 = scf.for %i = %c1 to %c6 step %c1
    ...
    %new_wtok_a = amdgcn.store ds_write_b32 data %data_in addr %lds_addr_a offset c(%c0_i32)
      : ins(!amdgcn.vgpr, !amdgcn.vgpr, i32) -> !amdgcn.write_token<shared>
    amdgcn.wait deps %iter_wtok_a : !amdgcn.write_token<shared>
    %read_data, %rtok = amdgcn.load ds_read_b32 dest %s_read addr %lds_addr_a
      : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr) -> !amdgcn.read_token<shared>
    %new_wtok_b = amdgcn.store ds_write_b32 data %data_in addr %lds_addr_b offset c(%c0_i32)
      : ins(!amdgcn.vgpr, !amdgcn.vgpr, i32) -> !amdgcn.write_token<shared>
...
func.func @no_hoist_across_nested_loops(%addr: !amdgcn.vgpr<[? + 2]>, %lds_addr: !amdgcn.vgpr) {
  ...
  %res:2 = scf.for %i = %c1 to %c4 step %c1
    ...
    %init_wtok = amdgcn.store ds_write_b32 data %outer_data addr %lds_addr offset c(%c0_i32)
      : ins(!amdgcn.vgpr, !amdgcn.vgpr, i32) -> !amdgcn.write_token<shared>
    %inner_res = scf.for %j = %c0 to %c4 step %c1
      ...
      %new_inner_wtok = amdgcn.store ds_write_b32 data %outer_data addr %lds_addr offset c(%c0_i32)
        : ins(!amdgcn.vgpr, !amdgcn.vgpr, i32) -> !amdgcn.write_token<shared>
...
func.func @hoist_barrier_after_waits(%data_in: !amdgcn.vgpr, %lds_addr: !amdgcn.vgpr) {
  ...
  %c0_i32 = arith.constant 0 : i32
  %s_read = amdgcn.alloca : !amdgcn.vgpr
  %init_wtok = amdgcn.store ds_write_b32 data %data_in addr %lds_addr offset c(%c0_i32)
    : ins(!amdgcn.vgpr, !amdgcn.vgpr, i32) -> !amdgcn.write_token<shared>
  %init_data, %init_rtok = amdgcn.load ds_read_b32 dest %s_read addr %lds_addr
    : dps(!amdgcn.vgpr) ins(!amdgcn.vgpr) -> !amdgcn.read_token<shared>
  %res:2 = scf.for %i = %c1 to %c4 step %c1
