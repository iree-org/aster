...
// CHECK:           v_mov_b32 outs(%[[VAL_0]]) ins(%[[VAL_4]]) : outs(!amdgcn.vgpr<0>) ins(!amdgcn.vgpr<4>)
// CHECK:           test_inst outs %[[VAL_4]] : (!amdgcn.vgpr<4>) -> ()
// CHECK:           v_mov_b32 outs(%[[VAL_1]]) ins(%[[VAL_5]]) : outs(!amdgcn.vgpr<1>) ins(!amdgcn.vgpr<5>)
...
// CHECK:           s_mov_b32 outs(%[[VAL_4]]) ins(%[[VAL_3]]) : outs(!amdgcn.sgpr<2>) ins(!amdgcn.sgpr<1>)
...
// CHECK:           %[[VAL_5:.*]] = s_load_dword outs(%[[VAL_2]]) ins(%[[VAL_4]]) args(%[[CONSTANT_0]]) : outs(!amdgcn.sgpr<3>) ins(!amdgcn.sgpr<[0 : 2]>) args(i32) -> !amdgcn.read_token<constant>
...
// CHECK:           %[[VAL_6:.*]] = s_load_dwordx2 outs(%[[VAL_4]]) ins(%[[VAL_4]]) args(%[[CONSTANT_0]]) : outs(!amdgcn.sgpr<[0 : 2]>) ins(!amdgcn.sgpr<[0 : 2]>) args(i32) -> !amdgcn.read_token<constant>
...
amdgcn.kernel @test_index_bxmxnxk arguments <[#amdgcn.buffer_arg<address_space = generic>, #amdgcn.block_dim_arg<x>]> {
  ...
  %4 = alloca : !amdgcn.sgpr<?>
  %token = amdgcn.s_load_dword outs(%4) ins(%3) args(%c42_i32)
      : outs(!amdgcn.sgpr<?>) ins(!amdgcn.sgpr<[0 : 2]>) args(i32) -> !amdgcn.read_token<constant>
  ...
  %token_1 = amdgcn.s_load_dwordx2 outs(%9) ins(%3) args(%c42_i32)
      : outs(!amdgcn.sgpr<[? : ? + 2]>) ins(!amdgcn.sgpr<[0 : 2]>) args(i32) -> !amdgcn.read_token<constant>
...
// CHECK:           amdgcn.v_mov_b32 outs(%[[ALLOCA_0]]) ins(%[[ALLOCA_1]]) : outs(!amdgcn.vgpr<0>) ins(!amdgcn.vgpr<1>)
// CHECK:           amdgcn.s_mov_b32 outs(%[[ALLOCA_2]]) ins(%[[ALLOCA_3]]) : outs(!amdgcn.sgpr<0>) ins(!amdgcn.sgpr<1>)
...
// CHECK-DAG:       %[[ALLOCA_2:.*]] = amdgcn.alloca : !amdgcn.sgpr<0>
// CHECK-DAG:       %[[ALLOCA_3:.*]] = amdgcn.alloca : !amdgcn.sgpr<1>
// CHECK:           amdgcn.v_mov_b32 outs(%[[ALLOCA_0]]) ins(%[[ALLOCA_2]]) : outs(!amdgcn.vgpr<0>) ins(!amdgcn.sgpr<0>)
// CHECK:           amdgcn.v_mov_b32 outs(%[[ALLOCA_1]]) ins(%[[ALLOCA_3]]) : outs(!amdgcn.vgpr<1>) ins(!amdgcn.sgpr<1>)
// CHECK:           amdgcn.test_inst ins %[[ALLOCA_0]], %[[ALLOCA_1]], %[[ALLOCA_2]], %[[ALLOCA_3]] : (!amdgcn.vgpr<0>, !amdgcn.vgpr<1>, !amdgcn.sgpr<0>, !amdgcn.sgpr<1>) -> ()
// CHECK:           return
// CHECK:         }
...
// CHECK:           amdgcn.v_mov_b32 outs(%[[ALLOCA_0]]) ins(%[[ALLOCA_2]]) : outs(!amdgcn.vgpr<0>) ins(!amdgcn.vgpr<2>)
// CHECK:           amdgcn.v_mov_b32 outs(%[[ALLOCA_1]]) ins(%[[ALLOCA_3]]) : outs(!amdgcn.vgpr<1>) ins(!amdgcn.vgpr<3>)
// CHECK:           amdgcn.s_mov_b32 outs(%[[ALLOCA_4]]) ins(%[[ALLOCA_6]]) : outs(!amdgcn.sgpr<0>) ins(!amdgcn.sgpr<2>)
// CHECK:           amdgcn.s_mov_b32 outs(%[[ALLOCA_5]]) ins(%[[ALLOCA_7]]) : outs(!amdgcn.sgpr<1>) ins(!amdgcn.sgpr<3>)
...
// CHECK:           amdgcn.v_mov_b32 outs(%[[ALLOCA_2]]) ins(%[[ALLOCA_0]]) : outs(!amdgcn.vgpr<2>) ins(!amdgcn.vgpr<0>)
// CHECK:           amdgcn.v_mov_b32 outs(%[[ALLOCA_3]]) ins(%[[ALLOCA_1]]) : outs(!amdgcn.vgpr<3>) ins(!amdgcn.vgpr<1>)
// CHECK:           amdgcn.test_inst outs %[[ALLOCA_0]], %[[ALLOCA_1]] : (!amdgcn.vgpr<0>, !amdgcn.vgpr<1>) -> ()
...
func.func @redundant_copies() {
  ...
  amdgcn.s_mov_b32 outs(%2) ins(%3) : outs(!amdgcn.sgpr<?>) ins(!amdgcn.sgpr<?>)
  return
}
...
// CHECK:           v_mov_b32 outs(%[[VAL_1]]) ins(%[[VAL_2]]) : outs(!amdgcn.vgpr<1>) ins(!amdgcn.vgpr<2>)
// CHECK:           test_inst outs %[[VAL_1]] ins %[[VAL_2]] : (!amdgcn.vgpr<1>, !amdgcn.vgpr<2>) -> ()
// CHECK:           v_mov_b32 outs(%[[VAL_1]]) ins(%[[VAL_0]]) : outs(!amdgcn.vgpr<1>) ins(!amdgcn.vgpr<0>)
...
amdgcn.module @reg_alloc target = <gfx942> {
  ...
  // CHECK-NOT: v_mov
    ...
    %token = amdgcn.global_load_dword outs(%2) ins(%4) args(%c0_i32_mig1)
  ...
  // CHECK-NOT: v_mov
    ...
    %token = amdgcn.global_load_dword outs(%3) ins(%5) args(%c0_i32_mig2)
        : outs(!amdgcn.vgpr<?>) ins(!amdgcn.vgpr<[? : ? + 2]>) args(i32) -> !amdgcn.read_token<flat>
  ...
  ^bb1:  // 2 preds: ^bb0, ^bb1
    ...
    %token_0 = amdgcn.global_load_dword outs(%3) ins(%5) args(%c0_i32_mig3)
        : outs(!amdgcn.vgpr<?>) ins(!amdgcn.vgpr<[? : ? + 2]>) args(i32) -> !amdgcn.read_token<flat>
    lsir.copy %4, %3 : !amdgcn.vgpr<?>, !amdgcn.vgpr<?>
    cf.cond_br %0, ^bb1, ^bb2
  ^bb2:  // pred: ^bb1
