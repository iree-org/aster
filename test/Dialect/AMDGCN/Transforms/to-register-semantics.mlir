...
// CHECK:           %{{.*}} = amdgcn.s_load_dword outs(%[[ALLOCA_3]]) ins(%[[MAKE_REGISTER_RANGE_0]]) args(%[[CONSTANT_0]]) : outs(!amdgcn.sgpr<?>) ins(!amdgcn.sgpr<[0 : 2]>) args(i32) -> !amdgcn.read_token<constant>
...
// CHECK:           %{{.*}} = amdgcn.s_load_dwordx2 outs(%[[MAKE_REGISTER_RANGE_1]]) ins(%[[MAKE_REGISTER_RANGE_0]]) args(%[[CONSTANT_0]]) : outs(!amdgcn.sgpr<[? : ? + 2]>) ins(!amdgcn.sgpr<[0 : 2]>) args(i32) -> !amdgcn.read_token<constant>
...
func.func @test_index_bxmxnxk() {
  ...
  %4 = amdgcn.alloca : !amdgcn.sgpr
  %result, %token = amdgcn.s_load_dword outs(%4) ins(%3) args(%c42_i32)
  ...
  %result_0, %token_1 = amdgcn.s_load_dwordx2 outs(%9) ins(%3) args(%c42_i32)
