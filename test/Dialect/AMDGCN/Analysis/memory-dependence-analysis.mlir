...
amdgcn.module @test_memory_dependence target = #amdgcn.target<gfx942> isa = #amdgcn.isa<cdna3> {
  ...
  amdgcn.kernel @test_noalias_pointers {
    ...
    // Mark pointers as non-aliasing using pir.assume_noalias
    %ptr1_noalias, %ptr2_noalias = pir.assume_noalias %addr_range1, %addr_range2
      : (!amdgcn.sgpr_range<[? + 2]>, !amdgcn.sgpr_range<[? + 2]>)
      -> (!amdgcn.sgpr_range<[? + 2]>, !amdgcn.sgpr_range<[? + 2]>)
    ...
    // Load from ptr2 - because of pir.assume_noalias, this should NOT require
