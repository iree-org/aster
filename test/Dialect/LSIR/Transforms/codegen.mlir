...
// CHECK:           %[[LOAD:.*]],{{.*}} = amdgcn.global_load_dword outs(%[[DST]]) ins(%[[ADDR]])
...
// CHECK:           %[[LOAD:.*]],{{.*}} = amdgcn.global_load_dwordx2 outs(%[[DST]]) ins(%[[ADDR]])
...
// CHECK:           {{.*}} = amdgcn.global_store_dword ins(%[[DATA]], %[[ADDR]])
// CHECK:           return
// CHECK:         }
...
// CHECK:           {{.*}} = amdgcn.global_store_dwordx2 ins(%[[DATA]], %[[ADDR]])
// CHECK:           return
// CHECK:         }
...
// CHECK:           %[[DST:.*]] = lsir.alloca : !amdgcn.vgpr
// CHECK:           %[[LOAD:.*]],{{.*}} = amdgcn.ds_read_b32 outs(%[[DST]]) ins(%[[ADDR]])
// CHECK:           return %[[LOAD]] : !amdgcn.vgpr
// CHECK:         }
...
// CHECK:           %[[C0:.*]] = arith.constant 0 : i32
// CHECK:           %[[DST:.*]] = lsir.alloca : !amdgcn.vgpr<[? + 2]>
// CHECK:           %[[LOAD:.*]],{{.*}} = amdgcn.ds_read_b64 outs(%[[DST]]) ins(%[[ADDR]])
// CHECK:           return %[[LOAD]] : !amdgcn.vgpr<[? + 2]>
// CHECK:         }
...
// CHECK:           %[[C0:.*]] = arith.constant 0 : i32
// CHECK:           {{.*}} = amdgcn.ds_write_b32 ins(%[[ADDR]], %[[DATA]])
// CHECK:           return
// CHECK:         }
...
// CHECK:           %[[C0:.*]] = arith.constant 0 : i32
// CHECK:           {{.*}} = amdgcn.ds_write_b64 ins(%[[ADDR]], %[[DATA]])
// CHECK:           return
// CHECK:         }
