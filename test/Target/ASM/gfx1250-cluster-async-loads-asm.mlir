...
// Async counterpart of gfx1250-cluster-loads-asm.mlir (@cload).

// CHECK-LABEL: cload_async:
...
//       CHECK: cluster_load_async_to_lds_b32
...
amdgcn.module @cload_async_mod target = #amdgcn.target<gfx1250> {

  // scratch_buf (arg 0): 8 * 128 int32 (read_write; phase-1 producer).
  // output_buf  (arg 1): 8 * 128 int32 (write_only; broadcast result).
  amdgcn.kernel @cload_async arguments <[
    ...
    // 1. producer: store global id (wg_id*128 + tid) into scratch[wg_id*128 + tid].
    ...
    // 2. cluster barrier (load-bearing): producer stores drain + all WGs rendezvous.
    %bf = amdgcn.token_barrier scope(#amdgcn.barrier_scope<cluster>) deps %st_fence
            : !amdgcn.fence_token

    // 3. async multicast load global -> LDS from the cluster-uniform address.
    // 3.a. M0 = 0b1111 = 15: broadcast to all 4 WGs in the cluster.
    %m0   = amdgcn.alloca : !amdgcn.m0<0>
    %mask = arith.constant 15 : i32
    amdgcn.s_mov_b32 outs(%m0) ins(%mask) : outs(!amdgcn.m0<0>) ins(i32)
    // 3.b. global cluster-uniform byte offset = (cluster_id*4*128 + tid) * 4.
    ...
    // 3.c. per-thread LDS byte offset = tid * 4 (destination row in this WG's LDS).
    %lds_off_idx  = affine.apply affine_map<(d0) -> (d0 * 4)>(%tid_idx)
    %lds_off_i32  = arith.index_cast %lds_off_idx : index to i32
    %lds_off_vgpr = lsir.to_reg %lds_off_i32 : i32 -> !amdgcn.vgpr
    // 3.d. async load: scratch[cluster_base*128 + tid] -> LDS[tid*4], broadcast.
    %async_tok = amdgcn.cluster_load_async_to_lds_b32 lds_dest %lds_off_vgpr
                   addr %scratch_ptr m0 %m0 offset d(%cu_off_vgpr) + c(%c0)
    ...
    // 4. read the broadcast value back from LDS, pinned after the async drain.
    %lds_dst = amdgcn.alloca : !amdgcn.vgpr
    %ld, %ds_tok = amdgcn.ds_load_b32 dest %lds_dst addr %lds_off_vgpr offset c(%c0)
    ...
    // 5. store output[wg_id*128 + tid] = broadcast value.
